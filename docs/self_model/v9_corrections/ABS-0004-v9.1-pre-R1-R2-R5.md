...
`[PROPOSED_CONSTRAINT]` Self-authored-unreviewed verification cannot satisfy
admission for high-consequence outputs. A review artifact changes
`verifier_lineage_status` to `self_authored_with_review` only when that
artifact is linked to the exact VerificationRun and establishes all of the
following for the reviewed property: the reviewer invocation and executor
identity; reviewer independence relative to the verifier and the output under
review with no unresolved or disqualified independence dimension; coverage of
the verifier version, rule/test version, content-addressed inputs, execution
environment, and result; and a positive review outcome. Naming or linking a
review artifact without those recorded findings does not change the status.
