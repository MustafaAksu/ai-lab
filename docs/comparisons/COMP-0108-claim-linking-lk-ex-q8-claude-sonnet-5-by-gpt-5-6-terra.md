# COMP-0108: Provider Comparison — Claim linking LK-EX-Q8-claude-sonnet-5-by-gpt-5-6-terra

## Metadata

- comparison_id: `COMP-0108`
- title: `Claim linking LK-EX-Q8-claude-sonnet-5-by-gpt-5-6-terra`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-64b37d8de3f33a7e.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0108", "target_id": "INV-64b37d8de3f33a7e"}, {"authoritative": false, "evidence": "docs/invocations/INV-ce67eaa789d7628f.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0108", "target_id": "INV-ce67eaa789d7628f"}]`
- created_at: `2026-07-29T06:40:36.854881+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q8-claude-sonnet-5-by-gpt-5-6-terra You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: Persisting InvocationAuthorization under the current DecisionRecord field layout is migration-required if the later DecisionRecord family resolution uses a different structure.
   quote: "existing authorization records may need structural migration to conform — this is not a validator change"
   extractor called it about: InvocationAuthorization DecisionRecord family shape (ABS-0004 v7 §4.13)

2. claim: Revising RoleDefinition vocabulary or required_verified_properties semantics after Slice D admission logic is built would require migration of stored role records and governed classifications citing them.
   quote: "those stored role records — and anything that already cites them as authority for a governed classification — need migration"
   extractor called it about: RoleDefinition three axes and required_verified_properties (ABS-0004 v7 §4.9)

3. claim: Changing AccountablePrincipal field semantics later would require migrating the persisted seed record and records chained to it.
   ...
   extractor called it about: AccountablePrincipal shape (ABS-0004 v7 §4.13)

4. claim: Party sameness for self_issued is decided solely by exact identifier equality, and broader sameness is not detected.
   ...
   extractor called it about: InvocationAuthorization self_issued party-sameness rule

5. claim: Adopting broader-sameness detection later would require recomputing or migrating previously persisted self_issued=false records that are same-party under the broader rule.
   ...
   extractor called it about: InvocationAuthorization self_issued stored flag

6. claim: The one-hop check_depth bound is validation-code-only so long as Slice D multi-hop checking is additive.
   quote: "As long as Slice D's multi-hop check is additive — new records/new `check_depth` values sitting alongside existing one-hop results — existing records remain valid instances and need no migration."
   extractor called it about: self-adjudication check_depth bound

7. claim: The one-hop check_depth bound becomes migration-required if the eventual multi-hop design reinterprets or restructures the existing check_depth enum rather than extending it.
   quote: "This only becomes migration-required if the eventual multi-hop design requires reinterpreting or restructuring the existing `check_depth` enum rather than extending it;"
   extractor called it about: self-adjudication check_depth enum

8. claim: The evidence set does not establish whether the eventual multi-hop design will require reinterpretation or restructuring of the existing check_depth enum.
   quote: "the evidence set doesn't show that design, so I can't rule it out."
   extractor called it about: future multi-hop self-adjudication check_depth design

9. claim: Adding refusal reasons or Slice D predicates is validation-code-only because existing records are not invalidated.
   ...
   extractor called it about: refusal-reason enumeration and predicate registry entries

10. claim: Changing the consequence default/modifier table is migration-required because past records store the prior effective-consequence computation as a governance fact.
   quote: "migration-required if the default/modifier table changes"
   extractor called it about: InvocationAuthorization consequence classification

11. claim: Adding new consequence modifiers only for future records is validation-code-only if already-issued records' stored effective class is unchanged.
   quote: "validation-code-only if only new modifiers are added going forward"
   extractor called it about: InvocationAuthorization consequence classification modifiers

12. claim: ABS-0004 v7 Section 8 is needed to determine whether stored effective-consequence values are tightly coupled to the classification table.
   quote: "needed to judge how tightly the stored effective-consequence value is coupled to the classification table"
   extractor called it about: ABS-0004 v7 Section 8 consequence classification detail

13. claim: ABS-0004 v7 Section 11 is needed to determine whether Slice D lineage work depends on OutboundInteractionLog's shape and whether it is schema-frozen.
...
14. claim: The full DecisionRecord resolution and COMP-0037 record are needed to confirm what field reconciliation remained unresolved before implementation.
   quote: "I'd want it to confirm exactly what field reconciliation was left unresolved before implementation"
   extractor called it about: [OPEN] DecisionRecord resolution and COMP-0037

15. claim: No schema-versioning or migration policy document for AI-Lab self_model records is in evidence.
   quote: "none is in evidence."
   extractor called it about: AI-Lab self_model schema-versioning and migration policy

16. claim: Without a schema-versioning or migration policy, the answer cannot confirm whether migration means a one-time backfill script or a harder irreversible-record problem.
   ...
   extractor called it about: meaning and reversibility of self_model record migration

17. claim: The ExecutorIdentity superclass question is unresolved and directly affects lineage and independence comparison across ModelIdentity, ToolIdentity, and HumanIdentity.
   quote: "which is unresolved and bears directly on lineage/independence comparison across `ModelIdentity`/`ToolIdentity`/`HumanIdentity`."
   extractor called it about: ExecutorIdentity superclass question (ABS-0004 v7 §4.7; COMP-0032)

18. claim: The evidence does not determine whether resolving ExecutorIdentity identity-equivalence semantics would be validation-code-only or migration-required.
   quote: "that determination isn't answerable from the evidence here"
   extractor called it about: ExecutorIdentity identity-equivalence semantics resolution
...
```
{"links":[{"claim":1,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":""},{"claim":3,"block":"ABS-0004:v7:S4.13#def:accountableprincipal-principal-id","reason_if_null":""},{"claim":4,"block":"PLAN-20260723-0002#mitigation","reason_if_null":""},{"claim":5,"block":"PLAN-20260723-0002#scope[11]","reason_if_null":""},{"claim":6,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":""},{"claim":7,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":""},{"claim":8,"block":null,"reason_if_null":"It concerns an unspecified future multi-hop design and whether it restructures check_depth; no candidate gives that design."},{"claim":9,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":10,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":""},{"claim":11,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":""},{"claim":12,"block":null,"reason_if_null":"It concerns the missing full ABS-0004 v7 Section 8 detail rather than a supplied candidate passage."},{"claim":13,"block":null,"reason_if_null":"It concerns the missing ABS-0004 v7 Section 11 and OutboundInteractionLog schema status."},{"claim":14,"block":null,"reason_if_null":"It concerns missing full DecisionRecord-resolution and COMP-0037 records needed to establish unresolved reconciliation."},{"claim":15,"block":null,"reason_if_null":"It concerns the absence of a schema-versioning or migration-policy document from the evidence."},{"claim":16,"block":null,"reason_if_null":"It concerns what a missing migration policy would establish about migration reversibility."},{"claim":17,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":""},{"claim":18,"block":null,"reason_if_null":"It concerns whether resolving the open ExecutorIdentity identity-equivalence question would require migration, which no candidate determines."}]}
```
...
{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":null},
{"claim":3,"block":"ABS-0004:v7:S4.13#def:accountableprincipal-principal-id","reason_if_null":null},
{"claim":4,"block":"PLAN-20260723-0002#mitigation","reason_if_null":null},
{"claim":5,"block":"PLAN-20260723-0002#scope[11]","reason_if_null":null},
{"claim":6,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":7,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":8,"block":null,"reason_if_null":"Concerns the extractor's own inability to determine future design details, not a specific source block."},
{"claim":9,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":10,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":null},
{"claim":11,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":null},
{"claim":12,"block":null,"reason_if_null":"Refers to needing to consult ABS-0004 v7 Section 8 text itself, which is not among the candidate blocks."},
{"claim":13,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":null},
{"claim":14,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":null},
{"claim":15,"block":null,"reason_if_null":"Concerns absence of a self_model schema-versioning/migration policy document not present among candidates."},
{"claim":16,"block":null,"reason_if_null":"Concerns interpretation of 'migration' for self_model records absent a policy document, not tied to a candidate block."},
