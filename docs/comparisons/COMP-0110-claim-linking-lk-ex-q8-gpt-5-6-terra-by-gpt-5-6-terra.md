# COMP-0110: Provider Comparison — Claim linking LK-EX-Q8-gpt-5-6-terra-by-gpt-5-6-terra

## Metadata

- comparison_id: `COMP-0110`
- title: `Claim linking LK-EX-Q8-gpt-5-6-terra-by-gpt-5-6-terra`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-dd8ed48d72e73631.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0110", "target_id": "INV-dd8ed48d72e73631"}, {"authoritative": false, "evidence": "docs/invocations/INV-33106d918a779181.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0110", "target_id": "INV-33106d918a779181"}]`
- created_at: `2026-07-29T06:41:57.038379+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q8-gpt-5-6-terra-by-gpt-5-6-terra You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: Neither proposed instrument has governance force.
   quote: "Neither proposed instrument has governance force."
   extractor called it about: the two proposed instruments

2. claim: Changing RoleDefinition axes, values, or required-property meanings would require updating role records and re-evaluating dependent admission decisions.
   ...
   extractor called it about: RoleDefinition axes and required_verified_properties semantics

3. claim: Adding inherited fields or splitting conflated DecisionRecord statuses requires backfill or explicit unknown values for old records.
   ...
   extractor called it about: DecisionRecord inherited fields and recommended_status/effective_status distinction

4. claim: EvidenceAdmissionDecision records admitted without lineage, independence, or verification fields require migration and re-adjudication.
   ...
   extractor called it about: EvidenceAdmissionDecision admission schema

5. claim: Resolving the warrant/admission record-family choice after records are created would require expensive existing-record migration.
   quote: "Expensive existing-record migration if records are created before resolution."
   extractor called it about: warrant/admission record family and subtype choice

6. claim: Recording self_issued and changing independence meanings for existing authorizations requires migration or derived-result backfill.
   quote: "Recording `self_issued` and changing the meaning of independence for existing authorizations requires **migration or derived-result backfill**."
   extractor called it about: self_issued authorization marking and independence semantics

7. claim: Extending one-hop self-adjudication traversal is initially validation-code only if complete lineage edges already exist.
   quote: "Extending traversal is initially **validation-code only** if complete lineage edges already exist."
   extractor called it about: one-hop self-adjudication check

8. claim: Composite-executor disclosure through spawned invocation links entails expensive existing-record migration and potentially irrecoverable historical gaps.
   quote: "Expensive existing-record migration, and potentially irrecoverable historical gaps."
   extractor called it about: composite-executor spawned invocation links

9. claim: Tool-configuration disclosure through EffectiveInputManifest entails expensive migration or backfill and may be impossible.
   quote: "Expensive migration/backfill; potentially impossible."
   extractor called it about: EffectiveInputManifest configuration, environment, and dependency disclosure

10. claim: External-interaction disclosure through OutboundInteractionLog entails expensive migration or backfill and may be impossible.
   quote: "Expensive migration/backfill; potentially impossible."
   extractor called it about: OutboundInteractionLog

11. claim: Section 8 is needed to determine the full consequence-class vocabulary, modifier set, and compatibility or migration consequences.
   quote: "Section 8, to determine the full consequence-class vocabulary, modifier set, and compatibility/migration consequences."
   extractor called it about: Section 8

12. claim: Section 11 is needed to determine OutboundInteractionLog shape, retention, and linkage rules.
   quote: "Section 11, to determine the required shape, retention, and linkage rules for `OutboundInteractionLog`."
   extractor called it about: Section 11 and OutboundInteractionLog

13. claim: Definitions of GraphRelation, predicate lists, and produced_by, executed_by, and spawned record shapes are needed to assess whether multi-hop traversal can be added without changing stored records.
   quote: "The definitions of `GraphRelation`, the Slice A/B/C predicate lists, and the `produced_by`, `executed_by`, and `spawned` record shapes"
   extractor called it about: GraphRelation, Slice A/B/C predicates, and lineage record shapes

14. claim: Definitions of C3, P1, P4, and P7 are needed to determine the precise scope of independence, authority, disclosure, and root-authority claims.
   quote: "The applicable definitions of C3, P1, P4, and P7, to determine the precise scope of independence, authority, disclosure, and root-authority claims."
   extractor called it about: C3, P1, P4, and P7 definitions

15. claim: Retention, versioning, supersession, and migration rules are needed to determine whether changed validation preserves historical outcomes or rewrites or reclassifies them.
   quote: "Any retention, versioning, supersession, and migration rules for existing records"
   extractor called it about: existing-record retention, versioning, supersession, and migration rules

16. claim: Existing warrant/admission record schemas are needed to resolve the open choice between extending them and creating a new DecisionRecord family.
   quote: "The existing warrant/admission record schemas, needed to resolve the expressly open choice between extending them and creating a new `DecisionRecord` family."
   extractor called it about: existing warrant/admission record schemas
...
```
{"links":[{"claim":1,"block":null,"reason_if_null":"It concerns the governance force of two proposed instruments, not a specific listed passage."},{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a"},{"claim":3,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc"},{"claim":4,"block":"ABS-0004:v7:S4.13#def:evidenceadmissiondecision-decision"},{"claim":5,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc"},{"claim":6,"block":"PLAN-20260723-0002#scope[11]"},{"claim":7,"block":"PLAN-20260723-0002#rationale[2]"},{"claim":8,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:composite-executor-disclosure-an-i"},{"claim":9,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:tool-configuration-disclosure-for"},{"claim":10,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an"},{"claim":11,"block":null,"reason_if_null":"It concerns the missing content of Section 8, whose full vocabulary and migration rules are not included."},{"claim":12,"block":null,"reason_if_null":"It concerns the missing content of Section 11 and its OutboundInteractionLog requirements."},{"claim":13,"block":null,"reason_if_null":"It concerns definitions and record shapes not provided in the candidate blocks."},{"claim":14,"block":null,"reason_if_null":"It concerns the applicable definitions of C3, P1, P4, and P7, which are not included."},{"claim":15,"block":null,"reason_if_null":"It concerns retention, versioning, supersession, and migration rules that are not provided."},{"claim":16,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc"}]}
```
...
```
{"links":[
{"claim":1,"block":null,"reason_if_null":"About two unspecified 'proposed instruments' and their lack of governance force; not tied to any specific candidate block."},
{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":""},
{"claim":3,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},
{"claim":4,"block":"ABS-0004:v7:S4.13#def:evidenceadmissiondecision-decision","reason_if_null":""},
{"claim":5,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},
{"claim":6,"block":"PLAN-20260723-0002#scope[11]","reason_if_null":""},
{"claim":7,"block":"PLAN-20260723-0002#scope[5]","reason_if_null":""},
{"claim":8,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:composite-executor-disclosure-an-i","reason_if_null":""},
{"claim":9,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:tool-configuration-disclosure-for","reason_if_null":""},
{"claim":10,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":""},
{"claim":11,"block":null,"reason_if_null":"References 'Section 8' of ABS-0004, which is not among the candidate blocks (no consequence-class vocabulary section provided)."},
{"claim":12,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":""},
{"claim":13,"block":"PLAN-20260723-0002#constraints[2]","reason_if_null":""},
{"claim":14,"block":null,"reason_if_null":"References definitions of C3, P1, P4, P7 which are not provided as distinct blocks in the candidate list."},
{"claim":15,"block":null,"reason_if_null":"General statement about retention/versioning/migration rules not tied to a specific block; no dedicated block covers this."},
{"claim":16,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""}
]}
```
