# SYNCOMP-0014: Comparison Synthesis — Finalize Unresolved Gating For L0 Retrieval Simulator

## Metadata

- synthesis_id: `SYNCOMP-0014`
- title: `Finalize Unresolved Gating For L0 Retrieval Simulator`
- created_at: `2026-07-06T08:08:16.295275+00:00`
- command: `scripts/synthesize_comparison.py --provider openai docs/comparisons/COMP-0023-finalize-unresolved-gating-for-l0-retrieval-simulator.md`
- source_comparison: `docs/comparisons/COMP-0023-finalize-unresolved-gating-for-l0-retrieval-simulator.md`
...
~~~text
1) Shared agreement
- Canonical schema path: manifest.diagnostics.l0_retrieval_simulator
- Allow a read-only compatibility alias: context_pack.selection_diagnostics.retrieval_candidates (deprecated/alias)
- selection_effect semantics: fixed to "none" for this slice (enum with a single admitted value)
- Selection adapter: defer entirely to a separate PLAN; no adapter integration in this slice
- Combine policy and normalization: require explicit configuration; no implicit defaults
- Schema families approved (read-only/diagnostic metadata only): header (except reranker dispute), embedding_model, bm25_params, dense_params, combine_policy, normalization, candidates, aggregates, guardrails
- Guardrails intent: zero side effects (side_effects_blocked true; no writes, mutations, or embedding creations)
- Diagnostic-only scope: no prompt inclusion, no context selection changes, no production embedding/index mutation, no reranking, no token budget enforcement/telemetry, no persistent write-back

2) Meaningful differences
- Header.reranker field:
  - OpenAI: approve field but constrain value to "none" only; metadata-only
  - Claude: reject field entirely to avoid implying out-of-scope capability
- Governance snippet completeness:
  - OpenAI: includes explicit “no selection adapter integration” and “no cross-namespace/redaction bypass”
  - Claude: omits explicit “no selection adapter integration” snippet; frames ACL/namespace/redaction as out of scope rather than explicit bypass prohibition
- Operational semantics language:
  - OpenAI: emphasize simulated/external inputs; no automatic L0 retrieval
  - Claude: describes a simulator runtime pipeline (query → BM25+dense → normalization → logging), which could be misread as executing retrieval
- GAP-0001 next-targets wording:
  - OpenAI: narrower, avoids external references
  - Claude: broader admission checklist; references ABS-0003 (external dependency not otherwise defined in artifact)

3) Stronger points from OpenAI
- Precise guardrails/prohibitions, including selection adapter prohibition and cross-namespace/redaction bypass prohibition
- Clear constraints on header fields: corpus_snapshot_id required if candidates exist; l0_index_namespace must be read-only; algorithm descriptive only
- Keeps reranker field but safely constrained to "none," aligning telemetry with prohibition language
- PLAN text tightly aligned with “no automatic L0 retrieval” by requiring simulated or externally provided inputs

4) Stronger points from Claude
- Reranker field rejection reduces implied capability creep
- Clean deferral of adapter interface (no no-op), minimizing scope creep
- Detailed success criteria and follow-on governance/verify artifacts layout
- Explicit reminder that future enum expansion for selection_effect should be separately governed
...
- 2. selection_effect semantics:
  - Allowed values: ["none"] only (fixed for this slice)
- 3. Schema fields (approved with constraints):
  - header: simulator_id, run_id, timestamp_utc, request_id, episode_id, manifest_id, manifest_version, query_text, query_kind, algorithm, reranker, corpus_snapshot_id, l0_index_namespace
    - Constraints:
      - reranker: allowed values ["none"] only; field MAY be omitted (treat omission as "none")
      - corpus_snapshot_id: required if candidates are present
      - l0_index_namespace: MUST reference a read-only snapshot namespace
      - algorithm: descriptive metadata only; no execution implied
  - embedding_model: name, version, dimension (metadata only; no embedding creation)
  - bm25_params: k1, b, tokenizer, stoplist (diagnostic metadata only)
  - dense_params: metric, top_k, ef_search (diagnostic metadata only; no index mutation)
  - combine_policy: method, weights.bm25, weights.dense (simulated scoring only)
  - normalization: method, per_field (diagnostic metadata only)
  - candidates: chunk_id, artifact_id, version, span.start_token, span.end_token, provenance_path, bm25_score, dense_score, retrieval_score, rank, match_reason, selection_effect, notes
    - Constraints: selection_effect must be "none"; retrieval_score and rank are diagnostics only
  - aggregates: total_candidates, top_k, score_stats, normalization_applied, warnings (approved)
  - guardrails: side_effects_blocked, writes_performed, index_mutations, embedding_creations
    - Required values: side_effects_blocked=true; writes_performed=0; index_mutations=0; embedding_creations=0
- 4. Selection adapter:
  - Decision: Defer entirely to a separate PLAN; no no-op or interface stubs in this slice
  - Warrant impact: Include explicit prohibition on any selection adapter integration or invocation
- 5. Combine policy defaults:
  - Combine policy: explicit required config only (no defaults)
  - Normalization: explicit required config only (no defaults)
- 6. GAP-0001 wording (exact replacement):
  - "Design a governed evaluation slice that admits a pluggable selection adapter and offline reranker evaluation against synthetic/read-only corpora, with strict write protection and no prompt/context changes. Prepare schema extensions for a richer selection_effect enum and reranker telemetry, gated behind explicit PLAN and WARRANT."
...
- 8. Final PLAN record (exact wording):
  - plan_id: PLAN-20260706-0002
  ...
  - proposed_change:
    - Introduce manifest.diagnostics.l0_retrieval_simulator with approved fields and constraints (selection_effect fixed "none"; reranker allowed only "none" or omitted).
  ...
  - risk:
    - Potential confusion between diagnostic retrieval_score and selection semantics; mitigated by canonical diagnostics path and fixed selection_effect "none".
    - Implied capability creep from presence of reranker field; mitigated by schema constraint to "none" and explicit WARRANT prohibition.
  - next_action:
...
- Reranker field signaling: Presence could be misread as capability; mitigate via schema-level enum constraint ["none"], optional field treated as "none," plus explicit "No reranking" warrant.
- Execution semantics: Claude’s pipeline phrasing could be interpreted as live retrieval; reaffirm no automatic L0 retrieval and require simulated or precomputed, read-only snapshot inputs only.
- External dependency: Claude references ABS-0003; not defined in artifact. Treat as out-of-scope unless separately admitted.
- Alias lifecycle: Read-only alias approved; deprecation/retirement timeline not specified. Require deprecation notice in docs to prevent drift.
- Namespace/ACL phrasing: Prefer explicit “no cross-namespace/redaction bypass” prohibition (OpenAI) over “out of scope” language to avoid ambiguity in audits.
- Persistency scope: Both intend ephemeral outputs; ensure implementation forbids write-back to any persistent audit/lineage store within this slice.

7) Suggested re-ask prompt
scripts/compare_providers.py --latest-context --require-admission --title Resolve reranker field and execution semantics for Diagnostic L0 Retrieval Simulator Finalize two clarifications while keeping all prior boundaries intact (no automatic L0 retrieval, no context selection/prompt changes, no production embeddings/index mutation, no reranking, no token budget telemetry, no adapter integration, no persistent write-back).

Decisions to confirm:
1) Header.reranker field:
   - Proposed: keep header.reranker but restrict allowed values to ["none"] only; field MAY be omitted (treat omission as "none").
   - Output EXACT: either "ACCEPT as constrained" or "REJECT field entirely". If REJECT, provide exact schema delta.

2) Execution semantics:
   - Confirm: simulator SHALL NOT perform live retrieval against production indices. Inputs MUST be simulated or from read-only snapshots; no index/network mutations.
   - Output EXACT: "CONFIRM no live retrieval" or "ADJUST" with exact permissible operations.

Re-state adopted items (for confirmation only; do not revise):
- Canonical path: manifest.diagnostics.l0_retrieval_simulator with read-only alias context_pack.selection_diagnostics.retrieval_candidates.
- selection_effect enum: ["none"] only.
- Combine policy and normalization: explicit config only; no defaults.
- Governance snippets (verbatim) included for: no prompt inclusion; no context selection changes; no production embeddings/index mutation; no reranking; no token budget telemetry; no selection adapter integration; no cross-namespace/redaction bypass; no persistent write-back.

Respond with:
- A. Clear yes/no per item 1–2.
- B. If “REJECT” or “ADJUST,” provide exact replacement schema text or warrant language only for the affected item(s).
- C. No additional scope changes.
~~~
