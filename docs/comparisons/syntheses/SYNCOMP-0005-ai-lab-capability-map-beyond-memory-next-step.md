# SYNCOMP-0005: Comparison Synthesis — AI-Lab Capability Map Beyond Memory Next Step

## Metadata

- synthesis_id: `SYNCOMP-0005`
- title: `AI-Lab Capability Map Beyond Memory Next Step`
- created_at: `2026-07-02T12:05:06.088437+00:00`
- command: `scripts/synthesize_comparison.py docs/comparisons/COMP-0009-ai-lab-capability-map-beyond-memory-next-step.md --provider openai --title AI-Lab Capability Map Beyond Memory Next Step`
- source_comparison: `docs/comparisons/COMP-0009-ai-lab-capability-map-beyond-memory-next-step.md`
...
~~~text
1) Shared agreement
- Topological context packing (C) should not come first. Both argue it depends on prior structure/semantics (OpenAI: on consistent episode L1s and ACL-aware selection; Claude: on a labeled graph with defined predicates).
- Provenance and citation integrity are essential. Both reference span-bounded citations to L0 chunks (OpenAI enforces cid@version|span; Claude highlights citation validation/provenance as a key concern).
- Access control/redaction matters. OpenAI centers deny-by-default ACLs and namespace isolation; Claude includes access-control semantics in the predicate set (e.g., redacts, grants_access).
- Sequencing and safety over speed. Both emphasize establishing the right foundations to avoid leakage, complexity, or expensive rework later.

2) Meaningful differences
- Choice of next step:
  - OpenAI: Choose (A) InteractionLog/L1 summaries now.
  - Claude: Choose (B) Predicate vocabulary + WarrantEdge/MemoryEdge schema now.
- Dependency view:
  - OpenAI: Edges and topological packing should rest on stable episode-level L1 provenance and validated citations.
  - Claude: L1 summaries should be generated into an already-defined warrant/edge schema to avoid later migration; topology requires labeled edges first.
- Risk framing:
  - OpenAI: Early ontology lock-in and migration overhead are risks if B precedes a stable summarization/provenance layer.
  - Claude: Schema changes after data collection are expensive; defining predicates/edges first prevents rework and unblocks both A and C.
- Plan completeness:
  - OpenAI: Provides end-to-end schemas, write paths, validators, storage/indexing, integration, triggers, tests, non-goals, risks, SLOs, exit criteria.
  - Claude: Begins B’s plan (predicate vocabulary and WarrantEdge schema) but is incomplete/truncated and lacks validators, integration, tests, and non-goals in the artifact.

3) Stronger points from OpenAI
- Detailed, shippable plan with clear P0–P2 sequencing, acceptance tests (unit, integration, negative), SLOs, and exit criteria.
- Safety-first mechanics: deny-by-default ACLs, namespace isolation for redacted/full, strict citation validators on cid@version|span, idempotency, and selection budgeting with explicit inclusion reasons.
- Near-term measurable benefits: improved retrieval quality, reduced prompt bloat, and actionable telemetry without waiting on graph semantics.

4) Stronger points from Claude
- Prevents downstream rework by defining predicate and edge schemas before generating data that would later require migration.
- Articulates why topology (C) needs semantic labels and edge types (e.g., dependency, supports) to guide traversal and budgeting.
- Proposes a predicate vocabulary with structural, temporal, epistemic, and access-control predicates, plus a typed Predicate schema; begins WarrantEdge schema to capture warrants and confidence.

5) Combined answer
Decision: Proceed with (A) InteractionLog/L1 summaries as the immediate next step, while explicitly capturing the structure needed to make (B) low-friction later.
- Ordering rationale:
  - Aligns with OpenAI’s argument that L1 summaries are a required, low-risk building block for selection, budgeting, and validation, and directly improve quality and safety now.
  - Addresses Claude’s rework concern by ensuring L1 artifacts include strict span-bounded citations and generation/provenance metadata so they can be deterministically lifted into future WarrantEdge/MemoryEdge records.

Action plan (derived from OpenAI’s P0–P2, focused on future compatibility with B):
- Implement OpenAI’s EpisodeL1Summary and InteractionLogEvent schemas, write path, and validators with:
  - Mandatory span-bounded ChunkReferences (cid@version|span).
  - Provenance fields (generation_model, generation_prompt_id).
  - Namespace/ACL isolation and citation integrity validation.
- Store EpisodeL1 with embeddings referenced (not inlined), index by episode_id/time, and integrate into selection with a small, explicit budget slice and inclusion_reason = "episode_l1".
- Triggers: end-of-episode, N-turn cadence, manual, TTL/coverage-based refresh; observability for generation rate, failures, cost/latency.
- Non-goals (now): no predicate vocabulary or edge graph; no topological packing.
- Rework mitigation for B (to honor Claude’s point):
  - Ensure L1 keeps a machine-readable list of cited chunk_ids + spans and episode_id to backfill DERIVED_FROM/SUPPORTS-style edges later.
  - Maintain idempotent content hashing and versioning to support edge creation audits.
  - Keep redacted/full segregation to allow access-control predicates to map cleanly in a later graph.

Tests and exit criteria: Use OpenAI’s acceptance tests and exit criteria as-is.

Note on (inference): As a low-effort hedge against later B, optionally draft a non-binding predicates.md sketch (names only, no persistence) to standardize terminology in prompts and logs without locking storage schemas. This is an inference-based mitigation, not present as a hard requirement in the artifacts.

6) Risks or missing assumptions
- If (B) lands soon after (A), some migration work will still be required; mitigation relies on the completeness of L1 citations/provenance (OpenAI covers this, but Claude’s concern persists).
- Claude’s B plan in the artifact is incomplete (e.g., WarrantEdge fields, validators, integration, tests), so the effort/risk for B-first is under-specified here.
- OpenAI’s plan assumes existing ACL/redaction and tokenizer pinning infrastructure are ready; if not, timelines slip.
- Potential L1 hallucinations are addressed by validators, but quality still depends on prompt discipline and model behavior.
- Topological packing (C) remains gated on whichever of A/B lands first plus stable ACL-aware selection; neither plan alone enables C now.

7) Suggested re-ask prompt
Decide A-first with an explicit bridge to B, or B-first with production-grade depth. Please respond with one of the two tracks and provide the same level of specificity:

Track 1 (A-first, bridge to B):
- Confirm EpisodeL1Summary and InteractionLogEvent schemas, validators, storage/indexing, selection integration, triggers, and tests per OpenAI’s plan.
- Specify the exact fields and mapping rules by which EpisodeL1Summary.citations and provenance will be lifted into future WarrantEdge/MemoryEdge records (e.g., DERIVED_FROM, SUPPORTS), including idempotent edge_id derivation.
- Define acceptance tests that validate the round-trip: L0 → L1 (with citations) → WarrantEdge graph (simulated) without information loss.

Track 2 (B-first, production-ready):
- Provide a complete predicate vocabulary and WarrantEdge/MemoryEdge schemas (including all fields, inverses, transitivity, validators), storage, and indexing.
- Show how L1 generation (A) will immediately write into these schemas without migration, including ACL/redaction handling and citation span validation.
- Supply end-to-end tests mirroring OpenAI’s depth (unit, integration, negative), SLOs, and exit criteria.

In both tracks, include:
- Non-goals for this increment.
- Risks and concrete mitigations.
- Gating criteria to start C (topological packing) after A/B are complete.
~~~
