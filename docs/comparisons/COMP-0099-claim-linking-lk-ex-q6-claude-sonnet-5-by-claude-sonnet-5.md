# COMP-0099: Provider Comparison — Claim linking LK-EX-Q6-claude-sonnet-5-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0099`
- title: `Claim linking LK-EX-Q6-claude-sonnet-5-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-d9fa66d848a28f44.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0099", "target_id": "INV-d9fa66d848a28f44"}, {"authoritative": false, "evidence": "docs/invocations/INV-3bc213c048fbebb9.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0099", "target_id": "INV-3bc213c048fbebb9"}]`
- created_at: `2026-07-29T06:33:11.740792+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q6-claude-sonnet-5-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: The plan's claim is only partially true, and the plan's phrasing overstates it.
   quote: "Short answer: only partially, and the plan's phrasing overstates it."
   extractor called it about: plan rationale claim about Slice C and row C6

2. claim: Slice C's scope delivers the dependency named in the row: RoleDefinition records on the three axes, RoleQualification, AccountablePrincipal, and InvocationAuthorization records.
   quote: "Slice C's scope delivers exactly that dependency: RoleDefinition records on the three axes"
   extractor called it about: Slice C scope vs C6 row dependency

3. claim: Slice C would satisfy the precondition named in the row, producing the role records that the activation condition names as where evidence begins.
   quote: "Slice C would satisfy the *precondition* named in the row — it produces the role records C6's activation condition names as where evidence begins."
   extractor called it about: C6 activation condition vs Slice C output

4. claim: Slice C would not produce the row's target enforcement, an artifact validator that checks C6 itself rather than merely records naming roles.
   quote: "The row's *target enforcement* is "artifact validator" — something that checks C6 itself, not merely records that name roles."
   extractor called it about: C6 target enforcement gap

5. claim: C6's actual constraint is an exercise restriction: an invocation may not have both adjudicator-level epistemic authority and above-read_only action authority exercised against the same target unless the adjudication is covered by an independent verification edge.
   quote: "an invocation may not have *both* adjudicator-level epistemic authority *and* above-read_only action authority exercised against the same target "unless the adjudication is covered by an independent verification edge.""
   extractor called it about: C6 constraint (ABS-0004:v7:C6)

6. claim: Checking C6 requires evaluating whether an independent verification edge covers the adjudication, which is not in Slice C's scope.
   quote: "evaluating whether an independent verification edge covers the adjudication — which is not in Slice C's scope"
   extractor called it about: Slice C scope vs independent verification edge check

7. claim: The plan's non-goals explicitly exclude multi-hop lineage traversal and independence assessment as Slice D work.
   quote: "The plan's non-goals explicitly exclude "multi-hop lineage traversal, independence assessment" as Slice D work"
   extractor called it about: plan non-goals / Slice D scope

8. claim: The one check Slice C does implement, the one-hop self-adjudication check, is scoped by identity (produced_by/executed_by collision) and the plan itself says it implements only C3's most direct case, disclaiming any broader independence claim.
   quote: "the one check Slice C does implement (the one-hop self-adjudication check) is scoped by identity — `produced_by`/`executed_by` collision — which the plan itself says implements only "C3's most direct case," disclaiming any broader independence claim"
   extractor called it about: Slice C one-hop self-adjudication check

9. claim: The role record Slice C would leave names the assignment (that a role holds adjudicator epistemic authority and repository_mutator/external_actuator action authority) but is not evidence that the exercise restriction was ever checked.
   quote: "it is not evidence that the *exercise restriction* (same invocation, same target, no independent verification edge) was ever checked"
   extractor called it about: role record artifact vs C6 exercise restriction evidence

10. claim: Per the matrix's own rule, manual enforcement counts only when the check leaves a named artifact, and the claim that a check happens is not evidence.
   quote: "manual enforcement counts only when the check leaves a named artifact; the claim that a check happens is not evidence."
   extractor called it about: enforcement matrix rule on manual enforcement

11. claim: No component in Slice C's scope performs the axis-collision check that C6 requires.
   quote: "no component in Slice C's scope performs that check"
   extractor called it about: Slice C scope vs axis-collision check

12. claim: The closest analog, one-hop self-adjudication, checks executor identity rather than epistemic/action axis co-exercise, and is explicitly disclaimed as not implementing even C3 in general, let alone C6.
   quote: "the closest analog (one-hop self-adjudication) checks executor identity, not epistemic/action axis co-exercise, and is explicitly disclaimed as not implementing even C3 in general, let alone C6"
   extractor called it about: one-hop self-adjudication check limitations

13. claim: The rationale's claim that 'Slice C is what lets that row cite something' is true only in the narrow sense that it supplies the named dependency (role records) the activation condition requires.
   quote: ""Slice C is what lets that row cite something" is true only in the narrow sense that it supplies the named dependency (role records) the activation condition requires"
   extractor called it about: plan rationale claim, narrow validity

14. claim: The claim is not true in the sense that Slice C gives the row a citable enforcement artifact for the C6 check itself; the target enforcement artifact validator remains unbuilt and part of what it would need, independent-verification-edge assessment, is out of scope until a later slice.
   quote: "It is not true in the sense the sentence implies, that Slice C gives the row a citable *enforcement* artifact for the C6 check itself; the target enforcement ("artifact validator") for the axis-exercise restriction remains unbuilt"
   extractor called it about: plan rationale claim, false broader implication

15. claim: CAP-0015, CAP-0016, and GAP-0005 are listed in the plan's evidence_ids but were not included, and the answer would have used them to check whether either capability defines a check closer to C6's actual axis-collision restriction than the one-hop self-adjudication check.
   quote: "CAP-0015 and CAP-0016 (the capabilities Slice C depends on) and GAP-0005 are listed in the plan's `evidence_ids` but not included here"
   extractor called it about: CAP-0015, CAP-0016, GAP-0005

16. claim: ABS-0004 v7 Section 4.13 (DecisionRecord fields) and Section 8 (consequence classification) are absent and, while not decisive for this question, are relevant to confirming exactly what InvocationAuthorization records would and wouldn't attest to.
   quote: "Also absent is ABS-0004 v7 Section 4.13 (DecisionRecord fields) and Section 8 (consequence classification), referenced by the plan's scope items — not decisive for this question but relevant to confirming exactly what InvocationAuthorization records would and wouldn't attest to."
   extractor called it about: ABS-0004 v7 Section 4.13 and Section 8

=== CANDIDATE BLOCKS ===

--- ABS-0004:v7:S9#def:normative-state-proposed-adopted-m ---
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#rationale[0]"},{"claim":2,"block":"PLAN-20260723-0002#summary"},{"claim":3,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m"},{"claim":4,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m"},{"claim":5,"block":"ABS-0004:v7:C6#whole"},{"claim":6,"block":"PLAN-20260723-0002#non_goals[2]"},{"claim":7,"block":"PLAN-20260723-0002#non_goals[2]"},{"claim":8,"block":"PLAN-20260723-0002#scope[5]"},{"claim":9,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m"},{"claim":10,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m"},{"claim":11,"block":"PLAN-20260723-0002#proposed_change"},{"claim":12,"block":"PLAN-20260723-0002#scope[5]"},{"claim":13,"block":"PLAN-20260723-0002#rationale[0]"},{"claim":14,"block":"PLAN-20260723-0002#rationale[0]"},{"claim":15,"block":null,"reason_if_null":"The claim is about the omitted CAP-0015, CAP-0016, and GAP-0005 source documents and what they might establish, rather than a candidate passage."},{"claim":16,"block":null,"reason_if_null":"The claim is about absent ABS-0004 v7 Sections 4.13 and 8 and their unavailable content, not a supplied candidate block."}]}
```
...
```
{"links":[
{"claim":1,"block":null,"reason_if_null":"This is the answer's own overall verdict/reasoning summary, not a direct assertion about a specific source block."},
{"claim":2,"block":"PLAN-20260723-0002#summary","reason_if_null":null},
{"claim":3,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m","reason_if_null":null},
{"claim":4,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m","reason_if_null":null},
{"claim":5,"block":"ABS-0004:v7:C6#whole","reason_if_null":null},
{"claim":6,"block":"ABS-0004:v7:C6#whole","reason_if_null":null},
{"claim":7,"block":"PLAN-20260723-0002#non_goals[2]","reason_if_null":null},
{"claim":8,"block":"PLAN-20260723-0002#non_goals[4]","reason_if_null":null},
{"claim":9,"block":"ABS-0004:v7:C6#whole","reason_if_null":null},
{"claim":10,"block":"ABS-0004:v7:S9#def:normative-state-proposed-adopted-m","reason_if_null":null},
{"claim":11,"block":"ABS-0004:v7:C6#whole","reason_if_null":null},
{"claim":12,"block":"PLAN-20260723-0002#scope[5]","reason_if_null":null},
{"claim":13,"block":"PLAN-20260723-0002#rationale[0]","reason_if_null":null},
{"claim":14,"block":"PLAN-20260723-0002#rationale[0]","reason_if_null":null},
{"claim":15,"block":"PLAN-20260723-0002#evidence_ids[4]","reason_if_null":null},
{"claim":16,"block":null,"reason_if_null":"Refers to ABS-0004 v7 Section 4.13 and Section 8 content, which are not present among the candidate blocks (only S4.9 is included)."}
]}
```
