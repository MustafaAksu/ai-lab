# COMP-0109: Provider Comparison — Claim linking LK-EX-Q8-gpt-5-6-terra-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0109`
- title: `Claim linking LK-EX-Q8-gpt-5-6-terra-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-a6e81776e5f4e5ae.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0109", "target_id": "INV-a6e81776e5f4e5ae"}, {"authoritative": false, "evidence": "docs/invocations/INV-62cca326eafd4ef5.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0109", "target_id": "INV-62cca326eafd4ef5"}]`
- created_at: `2026-07-29T06:41:18.990905+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q8-gpt-5-6-terra-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: The answer states neither proposed instrument has governance force, so all decisions discussed are prospective design commitments, not admitted requirements.
   quote: "Neither proposed instrument has governance force."
   extractor called it about: governance status of the RoleDefinition/DecisionRecord instruments discussed

2. claim: Role definitions determine what later evidence admission must prove and which outputs can carry epistemic status; changing the axes, values, or meaning of a required property would require existing-record migration and re-evaluation of prior admission decisions, whereas adding a validator for an unchanged role schema is validation-code only.
   quote: "Changing axes, values, or the meaning of a required property would require updating role records and re-evaluating admission decisions that relied on those roles."
   extractor called it about: RoleDefinition axes and constraint fields (required_verified_properties, required_independence, requires_external_verification, may_assign_epistemic_status)

3. claim: Admission cannot infer acceptance authority from artifact production and must retain the distinction between executor, approver, and accountable principal; this requires existing-record migration if identities or fields have been collapsed in stored records, while a new validator enforcing an already-recorded separation is validation-code only.
   quote: "Admission cannot infer acceptance authority from artifact production, and later lineage work must retain the distinction between an executor, an approver, and an accountable principal."
   extractor called it about: distinction between task, epistemic, action, and accountable authority

4. claim: Later admission requires a record distinguishing a recommendation from accountable final status and identifying the evidence, authority, and supersession state relied on; adding inherited fields or splitting a previously conflated status is existing-record migration requiring backfill or explicit unknown values for old records.
   quote: "Adding inherited fields or splitting a previously conflated status changes persisted decision meaning and requires backfill or explicit unknown values for old records."
   extractor called it about: DecisionRecord inherited fields (recommended_status, effective_status, issuer invocation, accountable approver, evidence, policy authority, activation state, supersession)

5. claim: This is the direct future admission schema fixing the evidence needed to explain why an artifact or claim entered governed evidence; records admitted without these fields cannot be treated as if they contained lineage, independence, or verification findings, placing this in the existing-record migration and re-adjudication category, with validators alone able to enforce the rule only for new decisions.
   quote: "Records admitted without these fields cannot simply be treated as if they contained lineage, independence, or verification findings."
   extractor called it about: EvidenceAdmissionDecision fields (epistemic status, lineage/independence findings, provenance limitations, verified-property enumeration)

6. claim: This unresolved choice controls record identity, references, query patterns, supersession, and the relation between authorization and admission records; it would be an expensive existing-record migration if records are created before the choice is resolved, since a later family split, merge, or subtype conversion is not merely a validator change.
   quote: "A later family split, merge, or subtype conversion would require transforming records and their references."
   extractor called it about: open choice between extending existing warrant/admission records or creating a new record family, and whether kinds are subtypes or one family

7. claim: Admission and lineage work can rely only on qualifications tied to the executor's actual execution context; if the scope fields or transfer rule change, this requires existing-record migration/revalidation of old qualification records and authorizations, whereas rejecting new out-of-scope qualifications under the unchanged representation is validation-code only.
   quote: "A later admission cannot safely treat an executor as qualified merely because it was qualified elsewhere."
   extractor called it about: execution-scoped RoleQualification (endpoint, ExecutionProfile constraints, session-state constraints, policy version; no implicit transfer)

8. claim: Later admission has a recorded authority path distinguishing an in-scope authority claim from an unterminated or out-of-scope one; if scope or delegation semantics change this requires existing-record migration and re-evaluation because authorization-chain conclusions depend on them, though adding scope validation for records already containing the needed fields is validation-code only for future use, with historical governed classifications needing recomputation if retained as conclusions.
   quote: "historical governed classifications would need recomputation if they are retained as conclusions."
   extractor called it about: AccountablePrincipal authority_scope and delegation_reference, with authorization chains terminating at a recorded standing-authority claim within scope

9. claim: A self-issued authorization may exist but cannot supply the independent authorization a receiving role requires; recording self_issued and changing the meaning of independence for existing authorizations requires migration or derived-result backfill, while improving detection with the same stored identifiers is validation-code only prospectively, though prior independence findings need re-evaluation.
   quote: "A self-issued authorization may exist, but cannot supply the independent authorization that a receiving role requires."
   extractor called it about: self_issued marking and the rule that self-issued authorization cannot count as independent where independence is required

10. claim: Later lineage and independence work will miss collisions represented by different identifiers unless identity-equivalence work is added, bounding the evidentiary meaning of self-adjudication and self-issued results; this is primarily validation/identity-resolution code if original identifiers remain intact, but any historical independent or permitted one-hop result based on exact equality would require re-evaluation and likely migration of derived findings if broader equivalence is later adopted.
   quote: "Later lineage and independence work will miss collisions represented by different identifiers unless identity-equivalence work is added."
   extractor called it about: exact identifier equality as the plan's only party-sameness test, with broader sameness not detected

11. claim: The one-hop check supplies only the direct producer/executor collision check and does not establish multi-hop lineage independence, deferred to Slice D; extending traversal is initially validation-code only if complete lineage edges already exist, but old direct-only outcomes and downstream admission findings relying on them require re-evaluation or migration of derived records, and missing ancestry links cannot be repaired by code alone.
   quote: "Missing ancestry links cannot be repaired by code alone."
   extractor called it about: one-hop self-adjudication check marked check_depth as direct-only

12. claim: Future lineage must represent material model, tool, and human subexecutions rather than allowing authority or influence to disappear inside a tool or composite executor; new validation can reject undisclosed composition going forward, but it cannot reconstruct unrecorded subordinate executions in existing lineage, making this an expensive existing-record migration with potentially irrecoverable historical gaps.
   ...
   extractor called it about: composite-executor disclosure through spawned invocation links

13. claim: Future evidence admission and deterministic-standing assessments depend on knowing the effective inputs to a governed-evidence-producing tool; a validator can enforce manifests for new records, but historical configuration, environment, or dependency state may not be recoverable, making this an expensive migration/backfill that is potentially impossible.
   quote: "A validator can enforce manifests for new records, but historical configuration, environment, or dependency state may not be recoverable."
   extractor called it about: tool-configuration disclosure through EffectiveInputManifest (content-addressed configuration, environment, local dependencies)

14. claim: Later lineage and evidence admission must account for material external interactions and ambient consumers, not merely returned output bytes; enforcement can be added for future invocations, but absent historical interaction logs generally cannot be recreated reliably, making this an expensive migration/backfill that is potentially impossible.
   quote: "absent historical interaction logs generally cannot be recreated reliably"
   extractor called it about: external-interaction disclosure and OutboundInteractionLog

15. claim: Later lineage constraints may need polymorphic rules across models, tools, and humans, and the open superclass question affects how uniformly identity and executor checks can be expressed; this is a potentially expensive schema migration if records are first stored under one identity model and later require superclass or subtype-specific equivalence semantics, though implementing polymorphic validation without changing stored identity shapes could be validation-code only, depending on identity fields and relations not supplied in the material.
   quote: "that depends on identity fields and relations not supplied here"
   extractor called it about: invocation executor kinds remaining distinct while the ExecutorIdentity superclass question is open

16. claim: Admission and lineage records will need stable relation names and shapes for matters such as authorization, qualification, approval, production, execution, and ancestry; existing-record migration is required if persisted predicates or relation shapes are renamed, split, merged, or reinterpreted, whereas registering a new predicate without changing existing ones is generally schema/validation-code only.
   quote: "Admission and lineage records will need stable relation names and shapes for such matters as authorization, qualification, approval, production, execution, and ancestry."
   extractor called it about: fixed Slice C predicates and the requirement that relations use GraphRelation and registered predicate lists

17. claim: Later admission work inherits a provenance boundary in which an invocation without valid authorization is not retroactively made governed by this slice, limiting what future admission can claim about earlier records; if the boundary changes this requires reclassification and possibly migration of stored labels or derived decisions, though altering the classifier alone is validation-code only if historical classifications are retained as time-versioned results rather than overwritten.
   quote: "an invocation without valid authorization is not retroactively made governed by this slice"
   extractor called it about: governed versus experimental classification, with pre-slice invocations remaining experimental

18. claim: Consequence classes, role ceilings, and modifiers determine which roles and authorization conditions are applicable to later evidence-producing or admitting actions and are described as load-bearing for Slice D; existing-record migration and re-evaluation is needed if vocabulary ordering, defaults, modifier semantics, or exception requirements change, because stored authorization and admission conclusions may change, while adding checks for unchanged rules is validation-code only prospectively.
   quote: "They are described as load-bearing for Slice D."
   extractor called it about: consequence classes, role ceilings, modifiers that only raise, and controlled downward adjustment

19. claim: Because authorization is classification only, later evidence admission cannot equate execution occurrence with a successfully governed authorization check, preserving a population of executed but experimental invocations that Slice D may need to handle explicitly; this is primarily validation/workflow code to change prospectively, though if records or policies have relied on the distinction between executed, governed, and experimental, changing it would require reclassification of existing records but not necessarily rewriting invocation capture.
   quote: "Later evidence admission cannot equate execution occurrence with a successfully governed authorization check."
   extractor called it about: authorization being classification only and not refusing execution

20. claim: The decisions most expensive to reverse are those determining what was captured at execution time - material subordinate invocations, spawned lineage links, effective input manifests, and outbound interaction logs - because a later validator can reject future omissions but cannot reliably reconstruct omitted historical provenance; the next most expensive are record-family, role-vocabulary, identity, qualification-scope, and authority-chain choices.
   quote: "A later validator can reject future omissions, but cannot reliably reconstruct omitted historical provenance."
   extractor called it about: overall ranking of most expensive-to-reverse decisions across the slice
...
--- ABS-0004:v7:S4.9#def:a-role-composes-three-independent ---
A Role composes three independent axes:
...
--- ABS-0004:v7:S4.9#adopted_constraint:producing-an-artifact-does-not-gra ---
Producing an artifact does not grant authority to
accept it.

--- ABS-0004:v7:S4.9#adopted_constraint:witness-grounding-witness-outputs ---
Witness grounding: witness outputs must identify
...
--- ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a ---
RoleDefinition constraint fields: allowed input/output classes,
...
```
{"links":[{"claim":1,"block":null,"reason_if_null":"It concerns an answer-level statement about the governance force of proposed instruments, not a specific listed source block."},{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a"},{"claim":3,"block":"ABS-0004:v7:S4.9#def:a-role-composes-three-independent"},{"claim":4,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc"},{"claim":5,"block":"ABS-0004:v7:S4.13#def:evidenceadmissiondecision-decision"},{"claim":6,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc"},{"claim":7,"block":"ABS-0004:v7:S4.6-4.7#def:a-reified-policy-conclusion-that-q"},{"claim":8,"block":"PLAN-20260723-0002#scope[4]"},{"claim":9,"block":"PLAN-20260723-0002#scope[11]"},{"claim":10,"block":"PLAN-20260723-0002#mitigation"},{"claim":11,"block":"PLAN-20260723-0002#scope[12]"},{"claim":12,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:composite-executor-disclosure-an-i"},{"claim":13,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:tool-configuration-disclosure-for"},{"claim":14,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an"},{"claim":15,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe"},{"claim":16,"block":"PLAN-20260723-0002#constraints[2]"},{"claim":17,"block":"PLAN-20260723-0002#non_goals[3]"},{"claim":18,"block":"PLAN-20260723-0002#risk"},{"claim":19,"block":"PLAN-20260723-0002#non_goals[0]"},{"claim":20,"block":null,"reason_if_null":"It is an overall comparative ranking of reversibility and migration cost, rather than a claim about one specific listed block."}]}
```
...
```
{"links":[
{"claim":1,"block":null,"reason_if_null":"General statement about the overall proposed instruments' lack of governance force, not tied to a specific source block."},
{"claim":2,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":""},
{"claim":3,"block":"ABS-0004:v7:S4.9#adopted_constraint:producing-an-artifact-does-not-gra","reason_if_null":""},
{"claim":4,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},
{"claim":5,"block":"ABS-0004:v7:S4.13#def:evidenceadmissiondecision-decision","reason_if_null":""},
{"claim":6,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},
{"claim":7,"block":"ABS-0004:v7:S4.6-4.7#def:a-reified-policy-conclusion-that-q","reason_if_null":""},
{"claim":8,"block":"ABS-0004:v7:S4.13#def:accountableprincipal-principal-id","reason_if_null":""},
{"claim":9,"block":"PLAN-20260723-0002#scope[11]","reason_if_null":""},
{"claim":10,"block":"PLAN-20260723-0002#mitigation","reason_if_null":""},
{"claim":11,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":""},
{"claim":12,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:composite-executor-disclosure-an-i","reason_if_null":""},
{"claim":13,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:tool-configuration-disclosure-for","reason_if_null":""},
{"claim":14,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":""},
{"claim":15,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":""},
{"claim":16,"block":"PLAN-20260723-0002#constraints[2]","reason_if_null":""},
{"claim":17,"block":"PLAN-20260723-0002#non_goals[3]","reason_if_null":""},
{"claim":18,"block":"PLAN-20260723-0002#risk","reason_if_null":""},
{"claim":19,"block":"PLAN-20260723-0002#non_goals[0]","reason_if_null":""},
{"claim":20,"block":null,"reason_if_null":"This is a synthesized overall ranking across the entire plan/ontology, not about one specific source block."}
]}
```
