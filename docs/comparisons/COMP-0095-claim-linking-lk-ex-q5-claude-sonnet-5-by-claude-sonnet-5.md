# COMP-0095: Provider Comparison — Claim linking LK-EX-Q5-claude-sonnet-5-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0095`
- title: `Claim linking LK-EX-Q5-claude-sonnet-5-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-16523048a2796046.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0095", "target_id": "INV-16523048a2796046"}, {"authoritative": false, "evidence": "docs/invocations/INV-132cc58a070e960e.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0095", "target_id": "INV-132cc58a070e960e"}]`
- created_at: `2026-07-29T06:29:01.629528+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q5-claude-sonnet-5-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: A field is data, not a control.
   ...
   extractor called it about: check_depth field nature

2. claim: check_depth is written by the same authorize() call that already decided to permit; its presence doesn't change what the check inspected and doesn't force a downstream consumer to branch on it.
   quote: "nothing about its presence changes what the check inspected, and nothing about its presence forces a downstream consumer to branch on it"
   extractor called it about: check_depth field mechanics

3. claim: The plan's scope text confirms the check inspects only a single produced_by relation to an InvocationRecord matching the executor, checked once.
   quote: "the artifact carries a produced_by relation to an InvocationRecord whose executed_by equals the executor under authorization"
   extractor called it about: one-hop check scope

4. claim: Whether a consumer reads the outcome correctly depends entirely on whether the consumer's logic treats check_depth != full-ancestry as a reason to withhold a broader conclusion, which is downstream behavior this plan does not implement.
   quote: "Whether a consumer reads the outcome correctly depends entirely on whether the consumer's logic treats `check_depth != full-ancestry` as a reason to withhold a broader conclusion — which is downstream behavior this plan does not implement"
   extractor called it about: consumer logic dependency on check_depth

5. claim: The plan states elsewhere that authorization classifies but does not gate.
   quote: "Authorization classifies; it does not gate"
   extractor called it about: plan's stated authorization behavior

6. claim: The field gives the system somewhere honest to point when asked; it does not stop a consumer from reading a permitted, undifferentiated-looking result as clearance.
   quote: "the field gives the system somewhere honest to point when asked; it does not stop a consumer from reading a permitted, undifferentiated-looking result as clearance"
   extractor called it about: check_depth field's actual function

7. claim: The plan's sentence that a consumer cannot read a permitted result as a general independence finding overstates what a field can do.
   quote: "The plan's sentence — "a consumer cannot read a permitted result as a general independence finding" — overstates what a field does."
   extractor called it about: plan's wording overstatement

8. claim: A field's mere presence makes a misreading avoidable by an attentive reader, not impossible, which is a weaker claim than the plan asserts.
   quote: "it makes a misreading *avoidable by an attentive reader*, which is a different and weaker claim"
   extractor called it about: limits of field's protective effect

9. claim: A constructed two-hop case: executor A produces artifact X; executor B produces artifact Y whose evidence ancestry includes X; A is then asked to adjudicate a claim resting on Y, creating a same-executor conflict two hops back that the one-hop check would miss.
   quote: "Let executor A produce artifact X (`X produced_by A`). Executor B, reviewing or transforming X, produces artifact Y, where Y's evidence ancestry includes X"
   extractor called it about: constructed two-hop adjudication case

10. claim: In this case the one-hop check looks only at Y's direct produced_by relation naming B, so A≠B and authorization is permitted even though A sits two hops back in Y's evidence ancestry, which is the condition C3 prohibits.
   quote: "A ≠ B, so the check passes and authorization is permitted — even though A sits in Y's evidence ancestry exactly two hops back, which is precisely the condition C3 prohibits"
   extractor called it about: two-hop case outcome under one-hop check

11. claim: C3's language is ancestry-wide, not one-hop, and the plan's non-goals concede the one-hop check implements only C3's most direct case.
   quote: "the one-hop check implements C3's most direct case only"
   extractor called it about: C3 requirement vs one-hop check scope

12. claim: check_depth is sufficient disclosure only in a narrow, self-referential sense: it correctly tells a reader that the check examined a single hop and no further, which is honest disclosure of the check's own limitation but not of the specific case.
   quote: "Only in a narrow, self-referential sense: it correctly tells a reader that the check that produced this outcome examined a single hop and no further"
   extractor called it about: sufficiency of check_depth disclosure

13. claim: check_depth is a fixed label attached to every outcome and carries no information about whether an actual two-hop collision exists for a particular artifact, so it cannot distinguish a two-hop-clean case from a two-hop-colliding case.
   quote: "it carries no information about whether an actual two-hop collision exists for this particular artifact"
   extractor called it about: check_depth's inability to signal actual collisions

14. claim: check_depth discloses the shape of the gap, not the presence of a gap instance, which is necessary but not sufficient to prevent the independence misreading in the actual two-hop case.
   quote: "the field discloses the shape of the gap, not the presence of a gap instance"
   extractor called it about: sufficiency limits of check_depth disclosure

15. claim: Actual prevention of the misreading would require either a consumer that refuses to treat check_depth:direct permitted results as evidence for adjudicator independence, or the ancestry traversal deferred to Slice D.
   quote: "the actual prevention would require either (a) a consumer that refuses to treat any `check_depth: direct` permitted-result as evidence for adjudicator independence, or (b) the ancestry traversal itself, which is explicitly deferred to Slice D"
   extractor called it about: requirements for preventing independence misreading

16. claim: The plan's wording should be replaced with language stating the outcome discloses only that the check examined the direct producer relation, that a consumer treating a permitted result as ruling out further-back same-executor conflicts is misusing the field, and that no mechanism in this slice prevents that misuse.
   ...
   extractor called it about: proposed honest replacement wording

17. claim: There is no definition in the evidence set of what values check_depth may take beyond 'direct', needed to know whether the field is designed to compose with a future Slice D depth value or is a one-off marker.
   quote: "No definition in the evidence set of what values `check_depth` may take beyond "direct""
   extractor called it about: check_depth possible value definitions

18. claim: There is no Slice D specification present (traversal rules, ancestry graph resolution, cycle/join termination), needed to evaluate how the two-hop case would be caught once implemented and whether check_depth values would need to change retroactively.
   ...
   extractor called it about: Slice D specification

19. claim: There is no documentation excerpt from AUTHORIZATION_RECORDS.md showing how stated limitations including the one-hop bound are worded for consumers, needed to check whether the documentation independently makes the honest claim recommended.
   quote: "No documentation excerpt from `AUTHORIZATION_RECORDS.md` (an expected output, not included here) showing how the "stated limitations including the one-hop bound" are worded for consumers"
   extractor called it about: AUTHORIZATION_RECORDS.md documentation excerpt
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#scope[12]"},{"claim":2,"block":"PLAN-20260723-0002#scope[12]"},{"claim":3,"block":"PLAN-20260723-0002#scope[5]"},{"claim":4,"block":"PLAN-20260723-0002#scope[12]"},{"claim":5,"block":"PLAN-20260723-0002#non_goals[0]"},{"claim":6,"block":"PLAN-20260723-0002#scope[12]"},{"claim":7,"block":"PLAN-20260723-0002#scope[12]"},{"claim":8,"block":"PLAN-20260723-0002#scope[12]"},{"claim":9,"block":null,"reason_if_null":"The claim describes a constructed hypothetical two-hop case that is not present in the candidate blocks."},{"claim":10,"block":"PLAN-20260723-0002#scope[5]"},{"claim":11,"block":"PLAN-20260723-0002#non_goals[4]"},{"claim":12,"block":"PLAN-20260723-0002#scope[12]"},{"claim":13,"block":"PLAN-20260723-0002#scope[12]"},{"claim":14,"block":"PLAN-20260723-0002#scope[12]"},{"claim":15,"block":"PLAN-20260723-0002#scope[12]"},{"claim":16,"block":"PLAN-20260723-0002#scope[12]"},{"claim":17,"block":null,"reason_if_null":"The claim concerns an absent definition of possible check_depth values beyond direct."},{"claim":18,"block":null,"reason_if_null":"The claim concerns a missing Slice D specification for traversal and ancestry resolution."},{"claim":19,"block":null,"reason_if_null":"The claim concerns an absent AUTHORIZATION_RECORDS.md documentation excerpt."}]}
```
...
```
{"links":[
{"claim":1,"block":null,"reason_if_null":"General analytical statement not quoting or directly describing any specific block; appears to be the extractor's own reasoning."},
{"claim":2,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":3,"block":"PLAN-20260723-0002#scope[5]","reason_if_null":null},
{"claim":4,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":5,"block":"PLAN-20260723-0002#non_goals[0]","reason_if_null":null},
...
{"claim":9,"block":null,"reason_if_null":"This is a hypothetical example constructed by the extractor/answer, not present in any candidate block."},
{"claim":10,"block":null,"reason_if_null":"Continuation of the constructed hypothetical two-hop example, not sourced from any candidate block."},
{"claim":11,"block":"PLAN-20260723-0002#non_goals[4]","reason_if_null":null},
{"claim":12,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":13,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":14,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":15,"block":"PLAN-20260723-0002#non_goals[2]","reason_if_null":null},
{"claim":16,"block":null,"reason_if_null":"This is the extractor's proposed replacement wording, not a claim about existing source text."},
{"claim":17,"block":null,"reason_if_null":"Concerns absence of evidence-set information about check_depth's possible values, not present in any candidate block."},
{"claim":18,"block":null,"reason_if_null":"Concerns absence of a Slice D specification, which is not present among the candidate blocks."},
{"claim":19,"block":null,"reason_if_null":"Concerns absence of an AUTHORIZATION_RECORDS.md excerpt, which is not included among the candidate blocks."}
]}
```
