# COMP-0098: Provider Comparison — Claim linking LK-EX-Q5-gpt-5-6-terra-by-gpt-5-6-terra

## Metadata

- comparison_id: `COMP-0098`
- title: `Claim linking LK-EX-Q5-gpt-5-6-terra-by-gpt-5-6-terra`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-34b636250d5dda72.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0098", "target_id": "INV-34b636250d5dda72"}, {"authoritative": false, "evidence": "docs/invocations/INV-61ab9964dc8b37d1.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0098", "target_id": "INV-61ab9964dc8b37d1"}]`
- created_at: `2026-07-29T06:32:23.312349+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q5-gpt-5-6-terra-by-gpt-5-6-terra You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
2. claim: In the two-hop case, the proposed one-hop check permits authorization because the target artifact’s direct producing invocation B was executed by Y rather than X.
   quote: "Since B was executed by Y rather than X, it permits the authorization."
   extractor called it about: two-hop case target artifact direct `produced_by` invocation B

3. claim: In the two-hop case, the target artifact’s evidence ancestry containing A executed by X is a conflict under proposed C3.
   quote: "Under proposed C3, that is a conflict:"
   extractor called it about: two-hop case evidence ancestry invocation A under proposed C3

4. claim: X appearing one hop farther back does not make X independent.
   quote: "The fact that X appears one hop farther back does not make X independent."
   extractor called it about: two-hop case executor identity X

5. claim: `check_depth` is useful disclosure only if its semantics unambiguously state that the outcome reflects a direct-only check and that no conclusion about deeper ancestry follows.
   ...
   extractor called it about: `check_depth` semantics

6. claim: `check_depth` is not sufficient disclosure to support the proposition that a consumer cannot misread the result.
   quote: "It is not sufficient disclosure to support the stronger proposition that a consumer cannot misread the result."
   extractor called it about: `check_depth` disclosure

7. claim: A permitted result does not establish independence across untraversed lineage.
   quote: "a “permitted” result still does not establish independence across untraversed lineage."
   extractor called it about: permitted authorization result

8. claim: The plan’s wording should be replaced with wording that says the self-adjudication outcome records that only a direct check was performed.
   quote: "The self-adjudication outcome records that only a direct check was performed."
   extractor called it about: plan wording about the self-adjudication outcome

9. claim: The supplied evidence is sufficient to identify the conceptual two-hop miss and the overclaim.
   quote: "The supplied evidence is sufficient to identify the conceptual two-hop miss and the overclaim."
   extractor called it about: supplied evidence

10. claim: The supplied evidence is insufficient to determine whether the implementation would make the disclosure effective for downstream consumers.
   quote: "It is insufficient to determine whether the implementation would make the disclosure effective for downstream consumers."
   extractor called it about: implementation effectiveness of disclosure for downstream consumers

11. claim: The authorization-outcome schema and normative semantics for `check_depth`, including permitted values and whether it is mandatory, are missing material needed to assess disclosure effectiveness.
   ...
   extractor called it about: authorization-outcome schema and normative `check_depth` semantics

12. claim: The consumer/API/UI contract governing retention and interpretation of `check_depth` is missing material needed to assess disclosure effectiveness.
   quote: "the consumer/API/UI contract governing how consumers must retain and interpret `check_depth`; and"
   extractor called it about: consumer/API/UI contract for `check_depth`

13. claim: The Slice D lineage representation and traversal rules needed to detect the two-hop conflict are missing material.
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#scope[12]"},{"claim":2,"block":"PLAN-20260723-0002#scope[5]"},{"claim":3,"block":"ABS-0004:v7:C3#whole"},{"claim":4,"block":"ABS-0004:v7:C3#whole"},{"claim":5,"block":"PLAN-20260723-0002#scope[12]"},{"claim":6,"block":"PLAN-20260723-0002#scope[12]"},{"claim":7,"block":"PLAN-20260723-0002#risk"},{"claim":8,"block":"PLAN-20260723-0002#scope[12]"},{"claim":9,"block":null,"reason_if_null":"This is an assessment of the sufficiency of the supplied evidence, not a claim made in a listed block."},{"claim":10,"block":null,"reason_if_null":"This concerns missing implementation evidence about downstream consumer effectiveness, which no listed block assesses."},{"claim":11,"block":null,"reason_if_null":"This concerns missing authorization-outcome schema and normative check_depth semantics, rather than a listed passage."},{"claim":12,"block":null,"reason_if_null":"This concerns a missing consumer/API/UI retention and interpretation contract, which is not addressed by a listed block."},{"claim":13,"block":"PLAN-20260723-0002#non_goals[2]"}]}
```
...
{"claim":2,"block":"PLAN-20260723-0002#scope[5]","reason_if_null":null},
{"claim":3,"block":"ABS-0004:v7:C3#whole","reason_if_null":null},
{"claim":4,"block":"ABS-0004:v7:C3#whole","reason_if_null":null},
{"claim":5,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
...
{"claim":9,"block":null,"reason_if_null":"Meta-commentary about the sufficiency of supplied evidence for the extractor's own analysis, not about a specific source block."},
{"claim":10,"block":null,"reason_if_null":"Meta-commentary about the sufficiency of supplied evidence for assessing implementation effectiveness, not tied to a specific block."},
{"claim":11,"block":null,"reason_if_null":"References a missing authorization-outcome schema and normative check_depth semantics not present in any candidate block."},
{"claim":12,"block":null,"reason_if_null":"References a missing consumer/API/UI contract for check_depth not present in any candidate block."},
{"claim":13,"block":"PLAN-20260723-0002#non_goals[2]","reason_if_null":null}
]}
```
