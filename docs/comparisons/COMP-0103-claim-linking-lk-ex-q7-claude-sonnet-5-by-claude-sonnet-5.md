# COMP-0103: Provider Comparison — Claim linking LK-EX-Q7-claude-sonnet-5-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0103`
- title: `Claim linking LK-EX-Q7-claude-sonnet-5-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-88ef0d8121bd9259.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0103", "target_id": "INV-88ef0d8121bd9259"}, {"authoritative": false, "evidence": "docs/invocations/INV-dd2a02bb4a5b7048.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0103", "target_id": "INV-dd2a02bb4a5b7048"}]`
- created_at: `2026-07-29T06:37:50.898377+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q7-claude-sonnet-5-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: Criterion 1 (governed/experimental classification with fixtures for both outcomes) is mechanically checkable because the classify function and its two outcomes are fully specified in scope and the criterion requires only fixture demonstration of a binary condition.
   quote: "Checkable. The classify function and its two outcomes are fully specified in `scope`, and the criterion asks only for fixture demonstration of a binary condition."
   extractor called it about: success criterion 1 (classification fixtures)

2. claim: Criterion 2 (ten refusal reasons each exercised by fixture, no permitted outcome without matching authorization) is mechanically checkable because the ten reasons are enumerated, authorize() is specified as pure, and the pass conditions are binary and fixture-testable.
   quote: "Checkable. The ten reasons are enumerated in `scope`, `authorize()` is specified as pure, and "yields that reason" / "no fixture path returns permitted without a match" are both binary, fixture-testable conditions."
   extractor called it about: success criterion 2 (refusal reasons)

3. claim: Criterion 3 (chain-terminates-nowhere and authority-scope-violation refusals) is mechanically checkable because both are named refusal reasons over stored records testable via fixture construction.
   quote: "Checkable. Both conditions are named refusal reasons over stored records, testable by constructing the corresponding fixture."
   extractor called it about: success criterion 3 (chain-terminates and authority-scope refusals)

4. claim: Criterion 4's mechanical logic (refuse when executor identities match, permit when they differ, check_depth field present and marked direct-only) is testable against any fixture InvocationRecord and does not require a real one.
   quote: "The mechanical parts — refuse when executor identities match, permit when they differ, `check_depth` field present and marked direct-only — are testable against any fixture InvocationRecord; nothing about that logic requires a "real" one."
   extractor called it about: success criterion 4 identity-collision logic

5. claim: The 'demonstrated against a real captured InvocationRecord' sub-clause of criterion 4 supplies no record identifier, source, or acceptance rule distinguishing 'real captured' from synthetic, so no check can fail this clause specifically and it is not mechanically checkable.
   quote: "As written there is no check that can fail this clause specifically — a reviewer cannot tell whether a given fixture satisfies "real" or not."
   extractor called it about: success criterion 4 'real captured InvocationRecord' clause

6. claim: Criterion 5 (consequence classification) is checkable only in the narrow sense the plan itself claims, as enumerated-fixture coverage rather than proof of monotonicity, since Section 8's max-formula and the governed-exception constraint are testable for hand-assigned modifier levels.
   quote: "Checkable only in the narrow sense the plan itself claims ("enumerated-fixture coverage... not a proof of monotonicity")."
   extractor called it about: success criterion 5 consequence classification

7. claim: Because Section 8 marks the Detailed classification function as [OPEN], there is no adopted rule for deriving which modifier levels apply to an invocation's actual content, so fixtures cannot check that classification of any real invocation is correct, only internal consistency of the combination formula.
   quote: "they cannot check that the classification of any real invocation is correct, because the mapping from invocation facts to modifier levels doesn't yet exist as governance text"
   extractor called it about: success criterion 5 vs Section 8 [OPEN] classification function

8. claim: Criterion 6 (qualification execution-scope rejection) is mechanically checkable because Section 4.6 gives the scope fields explicitly and the relevant test conditions are testable against the validator.
   quote: "Checkable. Section 4.6 gives the scope fields explicitly (endpoint, ExecutionProfile constraints, session-state constraints, policy version); "lacking an execution scope" and "outside its scope" are both testable against the validator."
   extractor called it about: success criterion 6 (execution-scope rejection)

9. claim: Criterion 7 (self-issued authorization accepted, self_issued true, reported not-independent) is checkable given the plan's stated 'exact identifier equality only' rule, which supplies the missing semantics.
   quote: "Checkable, given the plan's own stated rule (mitigation: "exact identifier equality only"). That rule supplies the missing semantics that a bare "same party" test would otherwise lack."
   extractor called it about: success criterion 7 (self-issued authorization)

10. claim: Criterion 8 (byte-identical Slice A/B outputs before and after regression) is mechanically checkable because byte-identical output is unambiguous and the fixture before/after procedure is stated.
   quote: "Checkable. "Byte-identical" is an unambiguous, mechanically comparable output; the fixture procedure (run before/after import) is stated."
   extractor called it about: success criterion 8 (regression check)

11. claim: The coverage criterion is not fully checkable as written because 'covered by tests exercising every public entrypoint and every enumerated outcome' supplies no completeness procedure, coverage tool, threshold, enumerated entrypoint list, or definition of 'enumerated outcome' beyond the ten refusal reasons.
   quote: "supplies no completeness procedure: no coverage tool, no threshold, no enumerated list of "public entrypoints" against which fixtures are checked off, no definition of what counts as an "enumerated outcome""
   extractor called it about: success criterion 9 (coverage criterion)

12. claim: The 'full suite passes offline with no network access' portion of the coverage criterion is checkable as a pass/fail condition with no network calls.
   quote: "The full suite passes offline with no network access" is checkable (pass/fail, no network calls)."
   extractor called it about: success criterion 9 offline/no-network sub-clause

13. claim: The audit criterion's 'verified_current' clause is not mechanically checkable because no procedure, command, tool, or semantics for verified_current appears anywhere in the evidence set.
   quote: "no procedure, command, tool, or semantics for `verified_current` appears anywhere in the evidence set."
   extractor called it about: success criterion 10 audit criterion 'verified_current'

14. claim: An audit-tool specification or schema document defining what verified_current asserts and how it is computed is missing from the evidence and would be needed to check the audit criterion.
   quote: "an audit-tool specification or schema document (not present in evidence) that would define what `verified_current` asserts and how it's computed"
   extractor called it about: audit-tool specification for verified_current

15. claim: The cross-environment reproduction criterion is not mechanically checkable because no environments, build inputs, or normalization rules are specified, so reproduction cannot be executed or checked.
   quote: "no environments are named, no build inputs are specified, and no normalization rules (line endings, timestamps, path separators, toolchain/interpreter versions) are given."
   extractor called it about: success criterion 10 cross-environment reproduction

16. claim: An environment/build specification such as a CI matrix or reproducibility spec is missing from the evidence set and would be needed to check the cross-environment reproduction criterion.
   quote: "an environment/build specification (e.g., a CI matrix or reproducibility spec) that is not present in this evidence set."
   extractor called it about: environment/build specification for cross-environment reproduction

17. claim: No success criterion tests scope containment against a warrant, so the constraint that implementation is authorized only within the scope admitted by the future admission warrant has no corresponding check among the ten criteria.
   quote: "No success criterion tests scope containment against a warrant; there is nothing to check this against until a warrant exists, and none of the ten criteria references warrant scope at all."
   extractor called it about: warrant-scope constraint vs success criteria

18. claim: No success criterion checks field-name/semantic conformance to ABS-0004 v7, since field reconciliation appears in scope as a deliverable rather than as a checked outcome in success_criteria.
   quote: "No criterion checks field-name/semantic conformance to the ontology text. "Field reconciliation with ABS-0004 4.13" appears in `scope`, not in `success_criteria` — it is a deliverable, not a checked outcome."
   extractor called it about: ABS-0004 v7 field-name/semantics constraint vs success criteria

19. claim: No success criterion checks predicate-registry conformance or GraphRelation shape, since predicate-registry entries appear only as an expected output rather than something any criterion verifies.
   quote: "No criterion checks predicate-registry conformance or relation shape; the predicate-registry entries appear as an expected output, not as something any success criterion verifies."
   extractor called it about: GraphRelation shape / predicate-registry constraint vs success criteria

20. claim: Only the network-access half of the authorize() purity constraint is indirectly covered by criterion 9; the 'no I/O', 'no clock', and 'timestamp supplied by caller' parts have no corresponding fixture or check in the ten criteria.
   quote: ""No I/O," "no clock," and "timestamp supplied by caller" have no corresponding fixture or check anywhere in the ten criteria."
   extractor called it about: authorize() purity constraint vs success criteria
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#success_criteria[0]","reason_if_null":""},{"claim":2,"block":"PLAN-20260723-0002#success_criteria[1]","reason_if_null":""},{"claim":3,"block":"PLAN-20260723-0002#success_criteria[2]","reason_if_null":""},{"claim":4,"block":"PLAN-20260723-0002#success_criteria[3]","reason_if_null":""},{"claim":5,"block":"PLAN-20260723-0002#success_criteria[3]","reason_if_null":""},{"claim":6,"block":"PLAN-20260723-0002#success_criteria[4]","reason_if_null":""},{"claim":7,"block":"ABS-0004:v7:S8#def:ordinal-classes-low-medium-high-no","reason_if_null":""},{"claim":8,"block":"PLAN-20260723-0002#success_criteria[5]","reason_if_null":""},{"claim":9,"block":"PLAN-20260723-0002#success_criteria[6]","reason_if_null":""},{"claim":10,"block":"PLAN-20260723-0002#success_criteria[7]","reason_if_null":""},{"claim":11,"block":"PLAN-20260723-0002#success_criteria[8]","reason_if_null":""},{"claim":12,"block":"PLAN-20260723-0002#success_criteria[8]","reason_if_null":""},{"claim":13,"block":"PLAN-20260723-0002#success_criteria[9]","reason_if_null":""},{"claim":14,"block":null,"reason_if_null":"It concerns a missing audit-tool specification or schema for verified_current, which is not among the candidate blocks."},{"claim":15,"block":"PLAN-20260723-0002#success_criteria[9]","reason_if_null":""},{"claim":16,"block":null,"reason_if_null":"It concerns a missing CI matrix or environment/build reproducibility specification, which is not among the candidate blocks."},{"claim":17,"block":"PLAN-20260723-0002#constraints[0]","reason_if_null":""},{"claim":18,"block":"PLAN-20260723-0002#constraints[1]","reason_if_null":""},{"claim":19,"block":"PLAN-20260723-0002#constraints[2]","reason_if_null":""},{"claim":20,"block":"PLAN-20260723-0002#constraints[3]","reason_if_null":""}]}
```
...
{"claim":5,"block":"PLAN-20260723-0002#success_criteria[3]"},
{"claim":6,"block":"PLAN-20260723-0002#success_criteria[4]"},
{"claim":7,"block":"ABS-0004:v7:S8#def:ordinal-classes-low-medium-high-no"},
{"claim":8,"block":"PLAN-20260723-0002#success_criteria[5]"},
{"claim":9,"block":"PLAN-20260723-0002#success_criteria[6]"},
{"claim":10,"block":"PLAN-20260723-0002#success_criteria[7]"},
{"claim":11,"block":"PLAN-20260723-0002#success_criteria[8]"},
{"claim":12,"block":"PLAN-20260723-0002#success_criteria[8]"},
{"claim":13,"block":"PLAN-20260723-0002#success_criteria[9]"},
{"claim":14,"block":null,"reason_if_null":"Refers to a hypothetical missing audit-tool specification document, not present in the candidate list."},
{"claim":15,"block":"PLAN-20260723-0002#success_criteria[9]"},
{"claim":16,"block":null,"reason_if_null":"Refers to a hypothetical missing environment/build specification document, not present in the candidate list."},
{"claim":17,"block":"PLAN-20260723-0002#constraints[0]"},
{"claim":18,"block":"PLAN-20260723-0002#constraints[1]"},
{"claim":19,"block":"PLAN-20260723-0002#constraints[2]"},
{"claim":20,"block":"PLAN-20260723-0002#constraints[3]"}
]}
```
