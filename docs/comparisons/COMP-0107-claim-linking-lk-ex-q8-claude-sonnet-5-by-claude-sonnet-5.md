# COMP-0107: Provider Comparison — Claim linking LK-EX-Q8-claude-sonnet-5-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0107`
- title: `Claim linking LK-EX-Q8-claude-sonnet-5-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-870e5754e0e7220a.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0107", "target_id": "INV-870e5754e0e7220a"}, {"authoritative": false, "evidence": "docs/invocations/INV-984013eb65fd950f.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0107", "target_id": "INV-984013eb65fd950f"}]`
- created_at: `2026-07-29T06:39:56.318244+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q8-claude-sonnet-5-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: The DecisionRecord record-family shape (ABS-0004 v7 §4.13) is a decision that constrains later evidence-admission/lineage work, and it is currently instantiated by InvocationAuthorization even though the family design is marked open.
   quote: "Whether existing warrant/admission records are extended to fulfill this or a new record family is created, and whether kinds are subtypes or one record family."
   extractor called it about: ABS-0004 v7 §4.13 DecisionRecord family shape

2. claim: Once InvocationAuthorization records exist under one field layout and EvidenceAdmissionDecision is later built under whichever family design resolves the open item, existing authorization records may need structural migration rather than just a validator change.
   quote: "existing authorization records may need structural migration to conform — this is not a validator change, it is a change to persisted record shape"
   extractor called it about: InvocationAuthorization / DecisionRecord family reconciliation

3. claim: RoleDefinition's required_verified_properties field (§4.9) is the direct hook used by EvidenceAdmissionDecision, and seed role records populate this field now, making the role vocabulary load-bearing for later admission logic.
   quote: "Role and consequence vocabularies decided here are load-bearing for Slice D's admission rules, and a coarse role taxonomy would propagate."
   extractor called it about: ABS-0004 v7 §4.9 RoleDefinition required_verified_properties

4. claim: Seed role records are persisted artifacts, so if the role vocabulary or required_verified_properties semantics needs revision once Slice D's admission logic is built, those stored records and anything citing them need migration, not just a validator update.
   quote: "those stored role records — and anything that already cites them as authority for a governed classification — need migration, not just a validator update"
   extractor called it about: seed RoleDefinition records in docs/roles/

5. claim: AccountablePrincipal shape (§4.13) is seeded now with a real operator record, and changing principal_id/principal_kind/authority_scope/delegation_reference semantics later would require migrating the persisted seed record and anything chaining to it.
   quote: "Changing `principal_id`/`principal_kind`/`authority_scope`/`delegation_reference` semantics later means migrating the persisted seed record and anything chaining to it."
   extractor called it about: ABS-0004 v7 §4.13 AccountablePrincipal shape

6. claim: self_issued is computed by exact-identifier party sameness only, with broader sameness not detected, and this flag is stored per authorization record at issue time rather than recomputed on demand.
   quote: "Party sameness is decided on exact identifier equality only, and the documentation states that broader sameness is not detected."
   extractor called it about: self_issued exact-identifier party sameness computation

7. claim: If broader-sameness detection is adopted later, previously persisted self_issued=false records that are actually same-party under the broader rule would be wrong and would need to be recomputed or migrated, since the flag was written once and consumed downstream as fact.
   ...
   extractor called it about: self_issued flag migration risk

8. claim: The self-adjudication outcome carries an explicit check_depth field naming it a direct check, so a consumer cannot read a permitted result as a general independence finding.
   quote: "the self-adjudication outcome carries an explicit `check_depth` field naming it a direct check, so a consumer cannot read a permitted result as a general independence finding"
   extractor called it about: self-adjudication check_depth field

9. claim: As long as Slice D's multi-hop check is additive, existing one-hop check_depth records remain valid instances and need no migration; this only becomes migration-required if the eventual multi-hop design requires reinterpreting or restructuring the existing check_depth enum, and the evidence set doesn't show that design so it can't be ruled out.
   quote: "this only becomes migration-required if the eventual multi-hop design requires reinterpreting or restructuring the existing `check_depth` enum rather than extending it; the evidence set doesn't show that design, so I can't rule it out"
   extractor called it about: check_depth one-hop bound future extension

10. claim: The refusal-reason enumeration and predicate registry entries are described as enumerable/extensible, so adding refusal reasons or Slice D predicates is additive to an enum/registry and does not invalidate existing records, only requiring validator/consumer updates.
   quote: "Nothing about existing records is invalidated by adding new reasons or predicates; only validators/consumers need updating."
   extractor called it about: refusal-reason enumeration and predicate registry

11. claim: Effective consequence classification is computed and stored per authorization at issue time as a decision fact, not derived live.
   quote: "Effective consequence is computed and stored per authorization at issue time (a decision fact), not derived live."
   extractor called it about: consequence classification storage

12. claim: Changing the default/modifier consequence classification table would be migration-required because past records embed the old computation as a governance fact, whereas adding only new modifiers going forward without touching already-issued records' stored class would be validation-code-only.
   quote: "migration-required if the default/modifier table changes**, since past records embed the old computation as a governance fact; **validation-code-only if only new modifiers are added going forward** without touching already-issued records' stored effective class"
   extractor called it about: consequence default/modifier table changes

13. claim: ABS-0004 v7 Section 8 (consequence classification detail) is needed to judge how tightly the stored effective-consequence value is coupled to the classification table, i.e., whether a table change is migration-required or re-derivable.
   quote: "needed to judge how tightly the stored effective-consequence value is coupled to the classification table, i.e., whether a table change is migration-required or re-derivable"
   extractor called it about: ABS-0004 v7 Section 8 consequence classification detail

14. claim: ABS-0004 v7 Section 11 (OutboundInteractionLog) is needed to judge whether Slice D lineage work depends on that log's shape and whether it is already schema-frozen.
   quote: "needed to judge whether Slice D lineage work depends on that log's shape and whether it's already schema-frozen"
   extractor called it about: ABS-0004 v7 Section 11 OutboundInteractionLog

15. claim: The full text of the [OPEN] DecisionRecord resolution and COMP-0037's record itself are needed to confirm exactly what field reconciliation was left unresolved before implementation, since that bears directly on migration cost for InvocationAuthorization records.
   quote: "I'd want it to confirm exactly what field reconciliation was left unresolved before implementation, since that bears directly on migration cost for InvocationAuthorization records"
   extractor called it about: COMP-0037 record and [OPEN] DecisionRecord resolution text

16. claim: No schema-versioning/migration policy document for AI-Lab's self_model records is in evidence, so it cannot be confirmed whether "migration" here means a one-time backfill script or a harder irreversible-record problem.
   quote: "Without it I can't confirm whether "migration" here means a one-time backfill script or a harder irreversible-record problem"
   extractor called it about: schema-versioning/migration policy document

17. claim: The answer does not treat the v6→v7 self-authorization/chain-termination framing as the only break relevant to this slice's lineage exposure; Section 4.7's ExecutorIdentity superclass question, reopened by COMP-0032, is a second unresolved construction bearing directly on lineage/independence comparison across ModelIdentity/ToolIdentity/HumanIdentity.
   quote: "a superclass with identity-equivalence semantics defined per subtype would make constraints such as C3 properly polymorphic across executor kinds"
   extractor called it about: ABS-0004 v7 §4.7 ExecutorIdentity superclass question (COMP-0032)

18. claim: Whether resolving the ExecutorIdentity superclass question later is validation-code-only or migration-required depends on whether identity-equivalence semantics can be added as a code-level interface over existing subtype records or requires new persisted fields, and this cannot be determined from the evidence here.
   quote: "that determination isn't answerable from the evidence here and is a second, independent risk from the one the plan foregrounds"
   extractor called it about: ExecutorIdentity superclass resolution migration category
...
```
{"links":[{"claim":1,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":""},{"claim":2,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":""},{"claim":3,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":""},{"claim":4,"block":"PLAN-20260723-0002#expected_outputs[5]","reason_if_null":""},{"claim":5,"block":"PLAN-20260723-0002#expected_outputs[5]","reason_if_null":""},{"claim":6,"block":"PLAN-20260723-0002#mitigation","reason_if_null":""},{"claim":7,"block":"PLAN-20260723-0002#scope[11]","reason_if_null":""},{"claim":8,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":""},{"claim":9,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":""},{"claim":10,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":11,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":""},{"claim":12,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":""},{"claim":13,"block":null,"reason_if_null":"The claim concerns the needed full detail of ABS-0004 v7 Section 8, which is not included as a candidate block."},{"claim":14,"block":null,"reason_if_null":"The claim concerns the needed full ABS-0004 v7 Section 11 OutboundInteractionLog text, which is not included as a candidate block."},{"claim":15,"block":null,"reason_if_null":"The claim concerns the missing COMP-0037 record and full unresolved-resolution material, neither of which is provided as a candidate block."},{"claim":16,"block":null,"reason_if_null":"The claim concerns a schema-versioning or migration-policy document that is not in the candidate list."},{"claim":17,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":""},{"claim":18,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":""}]}
```
...
```
{"links":[
{"claim":1,"block":"ABS-0004:v7:S4.13#def:decisionrecord-decision-kind-invoc","reason_if_null":null},
{"claim":2,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":null},
{"claim":3,"block":"ABS-0004:v7:S4.9#def:roledefinition-constraint-fields-a","reason_if_null":null},
{"claim":4,"block":"PLAN-20260723-0002#expected_outputs[5]","reason_if_null":null},
{"claim":5,"block":"ABS-0004:v7:S4.13#def:accountableprincipal-principal-id","reason_if_null":null},
{"claim":6,"block":"PLAN-20260723-0002#mitigation","reason_if_null":null},
{"claim":7,"block":"PLAN-20260723-0002#risk","reason_if_null":null},
{"claim":8,"block":"PLAN-20260723-0002#scope[12]","reason_if_null":null},
{"claim":9,"block":"PLAN-20260723-0002#scope[5]","reason_if_null":null},
{"claim":10,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":11,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":null},
{"claim":12,"block":"PLAN-20260723-0002#scope[6]","reason_if_null":null},
{"claim":13,"block":null,"reason_if_null":"Refers to ABS-0004 v7 Section 8 full text, which is not present among the candidate blocks."},
{"claim":14,"block":null,"reason_if_null":"Refers to ABS-0004 v7 Section 11 (OutboundInteractionLog) full text, which is not present among the candidate blocks."},
{"claim":15,"block":null,"reason_if_null":"Refers to the COMP-0037 record and the [OPEN] DecisionRecord resolution text, neither of which appear among the candidate blocks."},
{"claim":16,"block":null,"reason_if_null":"Refers to a schema-versioning/migration policy document for self_model records, which is not among the candidate blocks."},
{"claim":17,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":null},
{"claim":18,"block":"ABS-0004:v7:S4.6-4.7#def:an-invocation-is-an-event-performe","reason_if_null":null}
]}
```
