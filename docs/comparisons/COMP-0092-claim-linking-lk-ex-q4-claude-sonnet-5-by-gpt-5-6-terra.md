# COMP-0092: Provider Comparison — Claim linking LK-EX-Q4-claude-sonnet-5-by-gpt-5-6-terra

## Metadata

- comparison_id: `COMP-0092`
- title: `Claim linking LK-EX-Q4-claude-sonnet-5-by-gpt-5-6-terra`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-c9188033ba540a5f.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0092", "target_id": "INV-c9188033ba540a5f"}, {"authoritative": false, "evidence": "docs/invocations/INV-91115f35627b9e2a.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0092", "target_id": "INV-91115f35627b9e2a"}]`
- created_at: `2026-07-29T06:25:45.729180+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q4-claude-sonnet-5-by-gpt-5-6-terra You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: Completeness of the ten refusal reasons cannot be confirmed because the plan does not state which meaning of valid authorization they satisfy and leaves the Section 4.7 disclosure constraints undisposed.
   quote: "No — completeness cannot be confirmed, because the plan never states which meaning of "valid authorization" the ten reasons are meant to satisfy"
   extractor called it about: the plan's ten authorize() refusal reasons and Section 4.7 disclosure constraints

2. claim: The plan should state that Section 4.7 disclosure constraints are excluded from what authorize() checks in this slice.
   quote: "The plan should state that the Section 4.7 disclosure constraints are **excluded** from what `authorize()` checks in this slice"
   extractor called it about: Section 4.7 disclosure constraints in authorize()

3. claim: The plan should explicitly state that authorize() valid authorization means only coverage by an authorization record and does not certify disclosure compliance.
   quote: "The plan should say explicitly that `authorize()`'s "valid authorization" means only "covered by an authorization record," and does not certify disclosure compliance"
   extractor called it about: the meaning of valid authorization for authorize()

4. claim: The plan currently does not state the disposition of the Section 4.7 disclosure constraints.
   quote: "The plan currently says none of this."
   extractor called it about: the plan's disposition of Section 4.7 disclosure constraints

5. claim: An invocation with a Section 4.7 disclosure violation has no corresponding refusal reason among the ten.
   quote: "has no corresponding refusal reason among the ten."
   extractor called it about: the plan's ten refusal reasons and Section 4.7 disclosure violations

6. claim: Under the current ten reasons, an invocation with a required disclosure absent could pass every check and be classified governed.
   quote: "such an invocation could pass every check and be classified governed"
   extractor called it about: the plan's current ten authorize() refusal reasons

7. claim: Reason #9 reaches represented subordinates outside declared classes but does not reach a subordinate never disclosed as an Invocation.
   quote: "it does not reach a subordinate that was never disclosed as an Invocation at all"
   extractor called it about: refusal reason #9

8. claim: Reason #9 is the clearest enumerated reason broad enough to conceal materially distinct failures.
   quote: "Reason #9 is the clearest case."
   extractor called it about: refusal reason #9

9. claim: Collapsing declared scope gaps and undisclosed subordinate execution under one refusal reason would report the same code for materially different failures.
   quote: "Collapsing both under one refusal reason would report the same code for a remediable scope gap and for a disclosure violation"
   extractor called it about: refusal reason #9 and Section 4.7 disclosure violations

10. claim: Reason #2 is a secondary candidate for concealing distinct failures because it bundles at least three distinct scope dimensions.
   quote: "Reason #2 ("qualification out of execution scope") is a secondary candidate: it bundles at least three distinct scope dimensions"
   extractor called it about: refusal reason #2

11. claim: Section 4.9 and Section 4.13 are needed to confirm whether the ten reasons reconcile against every field required for a valid InvocationAuthorization.
   ...
   extractor called it about: Section 4.9, Section 4.13, and valid InvocationAuthorization fields

12. claim: Section 8 and Section 11 are needed; Section 11 would show whether the OutboundInteractionLog can feed authorize() at all.
   quote: "Section 11 in particular would show whether the log is structurally positioned to feed `authorize()` at all"
   extractor called it about: Section 8, Section 11, OutboundInteractionLog, and authorize()

13. claim: The COMP-0037 record is needed to determine whether disclosure-driven refusal was considered and knowingly omitted or simply not raised.
   quote: "needed to see the reviewers' own account of the four reasons added, and whether disclosure-driven refusal was considered"
   extractor called it about: COMP-0037

14. claim: GAP-0005, DECISION-20260723-0001, VERIFY-20260723-0002, CAP-0015, and CAP-0016 could contain the plan's adopted meaning of valid authorization.
   quote: "any of these could contain the "meaning of valid authorization the plan adopts""
   extractor called it about: GAP-0005, DECISION-20260723-0001, VERIFY-20260723-0002, CAP-0015, and CAP-0016
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":2,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":3,"block":"PLAN-20260723-0002#mitigation","reason_if_null":""},{"claim":4,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":5,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":6,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":7,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":8,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":9,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":10,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":""},{"claim":11,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":""},{"claim":12,"block":null,"reason_if_null":"It concerns the unavailable full text of Sections 8 and 11 and whether the OutboundInteractionLog can feed authorize()."},{"claim":13,"block":null,"reason_if_null":"It concerns the contents and review history of the unavailable COMP-0037 record."},{"claim":14,"block":null,"reason_if_null":"It concerns the contents of GAP-0005, DECISION-20260723-0001, VERIFY-20260723-0002, CAP-0015, and CAP-0016, which are not included as candidate blocks."}]}
```
...
{"claim":3,"block":"PLAN-20260723-0002#scope[7]","reason_if_null":null},
{"claim":4,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":5,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":6,"block":"PLAN-20260723-0002#scope[7]","reason_if_null":null},
...
{"claim":11,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":null},
{"claim":12,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":null},
{"claim":13,"block":"PLAN-20260723-0002#mitigation","reason_if_null":null},
{"claim":14,"block":null,"reason_if_null":"Claim discusses multiple separate evidence_id entries (GAP-0005, DECISION, VERIFY, CAP-0015, CAP-0016) collectively as unexamined sources for the plan's meaning of valid authorization, not the content of one specific block."}
]}
```
