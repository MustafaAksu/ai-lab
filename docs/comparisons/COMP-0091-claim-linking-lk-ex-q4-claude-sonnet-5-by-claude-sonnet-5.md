# COMP-0091: Provider Comparison — Claim linking LK-EX-Q4-claude-sonnet-5-by-claude-sonnet-5

## Metadata

- comparison_id: `COMP-0091`
- title: `Claim linking LK-EX-Q4-claude-sonnet-5-by-claude-sonnet-5`
- invocation_produced_by: `[{"authoritative": false, "evidence": "docs/invocations/INV-4a585b8ea5cf606a.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0091", "target_id": "INV-4a585b8ea5cf606a"}, {"authoritative": false, "evidence": "docs/invocations/INV-8f99629e5f407104.json", "predicate": "produced_by", "relation_source": "future_edge_seed", "scope": "invocation_provenance_slice_a", "source_id": "COMP-0091", "target_id": "INV-8f99629e5f407104"}]`
- created_at: `2026-07-29T06:24:52.823756+00:00`
- command: `scripts/compare_providers.py --title Claim linking LK-EX-Q4-claude-sonnet-5-by-claude-sonnet-5 You are linking extracted claims to the specific block of source text each claim is about. This is a matching task. Do not evaluate whether any claim is correct.
...
1. claim: Completeness cannot be confirmed, because the plan never states which meaning of "valid authorization" the ten reasons are meant to satisfy, and specifically leaves the 4.7 disclosure constraints undisposed.
   quote: "completeness cannot be confirmed, because the plan never states which meaning of "valid authorization" the ten reasons are meant to satisfy, and specifically leaves the 4.7 disclosure constraints undisposed"
   extractor called it about: plan's ten refusal reasons vs. meaning of valid authorization

2. claim: The plan should state that the Section 4.7 disclosure constraints are excluded from what authorize() checks in this slice — not checked here and not deferred to a named future slice.
   quote: "The plan should state that the Section 4.7 disclosure constraints are **excluded** from what `authorize()` checks in this slice — not "checked here" and not "deferred" to a named future slice."
   extractor called it about: disposition of Section 4.7 disclosure constraints in authorize()

3. claim: authorize() is constrained to be pure over stored records (no I/O, no clock, no network), and Slice A capture and Slice B resolution behavior are unchanged, so no field, path, or default of either is altered.
   quote: "pure over stored records: no I/O, no clock, no network"
   extractor called it about: authorize() function scope constraints

4. claim: The 4.7 disclosure constraints are enforced through artifacts the plan does not touch: representation as a spawned Invocation, the EffectiveInputManifest, and the OutboundInteractionLog; nothing in scope or expected_outputs builds or reads these.
   quote: "Nothing in `scope` or `expected_outputs` builds or reads these."
   extractor called it about: enforcement mechanism for Section 4.7 disclosure constraints

5. claim: "Deferred" would imply a named later slice picks this up, but the plan's non_goals list names what is deferred (multi-hop lineage, independence assessment, evidence admission — Slice D) and does not mention the 4.7 disclosure constraints anywhere.
   quote: ""Deferred" would imply a named later slice picks this up. The `non_goals` list names what is deferred (multi-hop lineage, independence assessment, evidence admission — Slice D) and does not mention the 4.7 disclosure constraints anywhere, including there."
   extractor called it about: plan's non_goals list vs. Section 4.7 disclosure constraints

6. claim: Section 3 already separates disclosure violations from authorization-scope violations for the one disclosure item reason #9 touches, stating undeclared subordinate execution is a disclosure violation under 4.7, not an implicitly authorized act.
   quote: "Undeclared subordinate execution is a disclosure violation under 4.7, not an implicitly authorized act."
   extractor called it about: Section 3 treatment of undeclared subordinate execution

7. claim: The plan should say explicitly that authorize()'s "valid authorization" means only "covered by an authorization record," and does not certify disclosure compliance, which is checked (if at all) elsewhere.
   quote: "The plan should say explicitly that `authorize()`'s "valid authorization" means only "covered by an authorization record," and does not certify disclosure compliance, which is checked (if at all) elsewhere."
   extractor called it about: definition of valid authorization the plan should state

8. claim: An invocation whose stored record itself carries a 4.7 disclosure violation — an undisclosed material subordinate execution never represented as a spawned Invocation, an undeclared execution-path-changing tool configuration, or an undeclared material external interaction — has no corresponding refusal reason among the ten.
   quote: "has no corresponding refusal reason among the ten"
   extractor called it about: missing refusal reason for stored-record 4.7 disclosure violations

9. claim: Reason #9 only reaches a subordinate execution that is represented but falls outside the declared classes; it does not reach a subordinate execution that was never disclosed as an Invocation at all, which 4.7 treats as categorically different.
   quote: "it does not reach a subordinate that was never disclosed as an Invocation at all, which 4.7 treats as categorically different"
   extractor called it about: refusal reason #9 scope limitation

10. claim: Under the current ten reasons, such an undisclosed invocation could pass every check and be classified governed, since nothing enumerated inspects for the absence of required disclosure.
   quote: "Under the current ten, such an invocation could pass every check and be classified governed, since nothing enumerated inspects for the absence of required disclosure."
   extractor called it about: gap in the ten enumerated refusal reasons

11. claim: Reason #9 is broad enough to conceal materially distinct failures: a declared, represented subordinate execution outside the parent's declared classes (a curable scope-boundary failure) versus a subordinate execution never represented as an Invocation at all (a disclosure violation not curable by a second authorization).
   quote: "Collapsing both under one refusal reason would report the same code for a remediable scope gap and for a disclosure violation with P4-level consequences"
   extractor called it about: refusal reason #9 conflating distinct failure types

12. claim: Reason #2 ("qualification out of execution scope") is a secondary candidate for concealing distinct failures, bundling at least three distinct scope dimensions — service endpoint, ExecutionProfile constraints, and session-state constraints.
   quote: "it bundles at least three distinct scope dimensions — service endpoint, ExecutionProfile constraints, and session-state constraints"
   extractor called it about: refusal reason #2 bundling distinct scope dimensions

13. claim: If 4.7's tool-configuration disclosure defeat is ever surfaced through a qualification's session-state field, that disclosure-driven failure would be reported identically to an ordinary endpoint/profile mismatch under reason #2.
   quote: "that disclosure-driven failure would be reported identically to an ordinary endpoint/profile mismatch"
   extractor called it about: refusal reason #2 potential concealment of disclosure failure

14. claim: Section 4.9 (RoleDefinition constraint fields) and Section 4.13 (DecisionRecord field set) are referenced by the plan's scope but not included, and are needed to confirm whether the ten reasons reconcile against every field the ontology requires a valid InvocationAuthorization to carry.
   quote: "needed to confirm whether the ten reasons reconcile against every field the ontology requires a valid `InvocationAuthorization` to carry"
   extractor called it about: Section 4.9 and Section 4.13

15. claim: Section 8 (consequence classification) and Section 11 (OutboundInteractionLog) are cited but not provided; Section 11 in particular would show whether the log is structurally positioned to feed authorize() at all, bearing on whether "excluded" is the right disposition.
   quote: "Section 11 in particular would show whether the log is structurally positioned to feed `authorize()` at all, which bears directly on whether "excluded" (vs. some future coupling) is the right disposition."
   extractor called it about: Section 8 and Section 11

16. claim: The COMP-0037 record itself, only summarized here, is needed to see the reviewers' own account of the four reasons added, and whether disclosure-driven refusal was considered and knowingly left out or simply not raised.
   quote: "needed to see the reviewers' own account of the four reasons added, and whether disclosure-driven refusal was considered and knowingly left out or simply not raised"
   extractor called it about: COMP-0037 record

17. claim: GAP-0005, DECISION-20260723-0001, VERIFY-20260723-0002, CAP-0015, and CAP-0016 are listed as evidence_ids but not supplied; any could contain the meaning of valid authorization the plan adopts, and its absence is why completeness cannot be certified.
   quote: "any of these could contain the "meaning of valid authorization the plan adopts" that the question asks me to test the ten reasons against, and its absence is exactly why completeness cannot be certified here"
   extractor called it about: unsupplied evidence_ids (GAP-0005, DECISION-20260723-0001, VERIFY-20260723-0002, CAP-0015, CAP-0016)
...
```
{"links":[{"claim":1,"block":"PLAN-20260723-0002#scope[4]"},{"claim":2,"block":"PLAN-20260723-0002#scope[4]"},{"claim":3,"block":"PLAN-20260723-0002#constraints[3]"},{"claim":4,"block":"PLAN-20260723-0002#scope[4]"},{"claim":5,"block":"PLAN-20260723-0002#non_goals[2]"},{"claim":6,"block":"ABS-0004:v7:S3#def:subordinate-authorization-inherita ---"},{"claim":7,"block":"PLAN-20260723-0002#mitigation"},{"claim":8,"block":"PLAN-20260723-0002#scope[4]"},{"claim":9,"block":"PLAN-20260723-0002#scope[4]"},{"claim":10,"block":"PLAN-20260723-0002#scope[4]"},{"claim":11,"block":"PLAN-20260723-0002#scope[4]"},{"claim":12,"block":"PLAN-20260723-0002#scope[4]"},{"claim":13,"block":"PLAN-20260723-0002#scope[4]"},{"claim":14,"block":"PLAN-20260723-0002#scope[10]"},{"claim":15,"block":null,"reason_if_null":"The claim concerns the absent full texts of Sections 8 and 11, which are not candidate blocks."},{"claim":16,"block":null,"reason_if_null":"The claim concerns the unsupplied COMP-0037 record itself, not a candidate block."},{"claim":17,"block":null,"reason_if_null":"The claim concerns the contents of unsupplied GAP, decision, verification, and capability records, rather than a listed candidate block."}]}
```
...
{"claim":3,"block":"PLAN-20260723-0002#constraints[3]","reason_if_null":null},
{"claim":4,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:composite-executor-disclosure-an-i","reason_if_null":null},
{"claim":5,"block":"PLAN-20260723-0002#non_goals[2]","reason_if_null":null},
{"claim":6,"block":"ABS-0004:v7:S3#def:subordinate-authorization-inherita","reason_if_null":null},
{"claim":7,"block":"PLAN-20260723-0002#scope[7]","reason_if_null":null},
{"claim":8,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":9,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":10,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":11,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":12,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":13,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":14,"block":"PLAN-20260723-0002#scope[10]","reason_if_null":null},
{"claim":15,"block":"ABS-0004:v7:S4.6-4.7#adopted_constraint:external-interaction-disclosure-an","reason_if_null":null},
{"claim":16,"block":"PLAN-20260723-0002#scope[4]","reason_if_null":null},
{"claim":17,"block":"PLAN-20260723-0002#evidence_ids[0]","reason_if_null":null}
]}
```
