...
def test_build_self_model_index_is_aggregation_only_for_seed_records():
    ...
    assert index["active_capabilities"] == ["CAP-0001", "CAP-0002", "CAP-0003", "CAP-0004", "CAP-0005", "CAP-0006", "CAP-0007"]
    assert index["open_gaps"] == ["GAP-0002"]
    assert index["capability_counts"]["implemented"] == 7
...
def test_build_self_model_script_writes_index(tmp_path):
    ...
    assert data["active_capabilities"] == ["CAP-0001", "CAP-0002", "CAP-0003", "CAP-0004", "CAP-0005", "CAP-0006", "CAP-0007"]
...
def test_build_self_model_index_includes_plan_records():
    ...
    assert index["plan_counts"]["completed"] == 12
    assert index["plan_counts"].get("admitted", 0) == 1
    assert index.get("plan_counts", {}).get("proposed", 0) == 0
    assert index["open_plans"] == ["PLAN-20260710-0001"]
    assert {
        plan["plan_id"]
        for plan in index["plans"]
    } == {"PLAN-20260705-0001", "PLAN-20260706-0001", "PLAN-20260706-0002", "PLAN-20260706-0003", "PLAN-20260706-0004", "PLAN-20260707-0001", "PLAN-20260707-0002", "PLAN-20260707-0003", "PLAN-20260707-0004", "PLAN-20260709-0001", "PLAN-20260709-0002", "PLAN-20260709-0003", "PLAN-20260710-0001"}
...
def test_build_self_model_index_records_verification_20260707_0003_for_threshold_completion():
    ...
    assert len(index["verifications"]) == 16
    assert any(
...
def test_gap_0002_recommended_slice_points_beyond_cap_0005_completion():
    ...
    assert record["status"] == "open"
    assert "CAP-0006" in record["recommended_first_slice"]
    assert "evidence-only" in record["recommended_first_slice"]
    assert "Do not modify context selection" in record["recommended_first_slice"]
    assert any(
...
def test_build_self_model_index_records_plan_20260709_0003_completed_final():
    ...
    assert index["open_plans"] == ["PLAN-20260710-0001"]
    assert index["open_gaps"] == ["GAP-0002"]
    assert index["plan_counts"] == {"admitted": 1, "completed": 12}
    assert len(index["verifications"]) == 16
    assert index["warrant_counts"]["supported"] == 30
    assert "PLAN-20260709-0003" in index["admitted_plans"]
    assert any(
...
def test_build_self_model_index_records_plan_20260710_0001_admitted():
    from ai_lab.documentation.self_model import build_self_model_index
...
def test_build_self_model_index_records_plan_20260710_0001_admission_warrant():
    from ai_lab.documentation.self_model import build_self_model_index
